/*
  ...
  Contributors:
    ...
    * Manuel Guilbault (code contributor, manuel.guilbault@g m a il.com):
      - line alignment.
  ...
  */
  public sealed class BlockComposer
  {
    ...
    private sealed class ContentPlaceholder
      : Operation
    {
      public List<ContentObject> _objects = new List<ContentObject>();
      ...
      {
        foreach(ContentObject obj in _objects)
    ...
    */
    #region fields
    private readonly PrimitiveComposer _baseComposer;
    private readonly ContentScanner _scanner;

    private bool _hyphenation;
    private char _hyphenationCharacter = '-';
    private LineAlignmentEnum _lineAlignment = LineAlignmentEnum.BaseLine;
    private Length _lineSpace = new Length(0, Length.UnitModeEnum.Relative);
    private XAlignmentEnum _xAlignment;
    private YAlignmentEnum _yAlignment;
    ...
    private RectangleF _boundBox;

    private Row _currentRow;
    private bool _rowEnded;

    private LocalGraphicsState _container;
    ...
    {
      this._baseComposer = baseComposer;
      this._scanner = baseComposer.Scanner;
    }
    #endregion
    ...
    */
      ...
      )
    {
      ...
      ContentScanner.GraphicsState state = _baseComposer.State;
      ...
      while(true)
      {
        if(_currentRow.Width == 0) // Current row has just begun.
        {
        ...
        if(OperationUtils.Compare(_currentRow.Y + lineHeight, _frame.Height) == 1) // Text's height exceeds block's remaining vertical space.
        {
        ...
        if(textFitter.Fit(
          index,
          _frame.Width - _currentRow.Width, // Remaining row width.
          _currentRow.SpaceCount == 0
          ))
        {
    ...
    */
      ...
      )
    {
      ...
      while(true)
      {
        if(OperationUtils.Compare(_currentRow.Y + size.Value.Height, _frame.Height) == 1) // Object's height exceeds block's remaining vertical space.
        {
          // Terminate current row and exit!
          EndRow(false);
          return false;
        }
        else if(OperationUtils.Compare(_currentRow.Width + size.Value.Width, _frame.Width) < 1) // There's room for the object in the current row.
        {
    ...
    */
      ...
      )
    {
      ...
      if(lineAlignment is double || lineAlignment.Equals(LineAlignmentEnum.BaseLine))
      {
        ...
        double superGap = obj.BaseLine + gap - _currentRow.BaseLine;
        if(superGap > 0)
        {
          _currentRow.Height += superGap;
          _currentRow.BaseLine += superGap;
        }
        double subGap = _currentRow.BaseLine + (obj.Height - obj.BaseLine) - gap - _currentRow.Height;
        if(subGap > 0)
        {_currentRow.Height += subGap;}
      }
      else if(obj.Height > _currentRow.Height)
    ...
    */
      ...
      )
    {
      ...
      _currentRow = new Row(
        (ContentPlaceholder)_baseComposer.Add(new ContentPlaceholder()),
        rowY
        );
    }
    ...
    */
      ...
      )
    {
      ...
      double[] objectXOffsets = new double[_currentRow.Objects.Count]; // Horizontal object displacements.
      double wordSpace = 0; // Exceeding space among words.
      double rowXOffset = 0; // Horizontal row offset.

      List<RowObject> objects = _currentRow.Objects;

      // Horizontal alignment.
      XAlignmentEnum xAlignment = this._xAlignment;
      switch(xAlignment)
      {
      ...
      _boundBox.Height = (float)(_currentRow.Y + _currentRow.Height);
      ...
      _boundBox.Y = (float)(_frame.Y + yOffset);
      ...
      )
    {
      ...
      if(lineAlignment is Length)
      {
        ...
        lineAlignment = ((Length)lineAlignment).GetValue(_lastFontSize);
      }
