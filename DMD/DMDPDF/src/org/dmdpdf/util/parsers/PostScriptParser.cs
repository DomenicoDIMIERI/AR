...
namespace org.dmdpdf.util.parsers
{
  /**
  ...
  */
    ...
    : IDisposable
  {
    ...
    */
    ...
    {this.stream = new org.dmdpdf.bytes.Buffer(data);}
    #endregion
        ...
        case Symbol.OpenRoundBracket: // Literal string.
                    {
                        ...
                        while(true) {
                            c = stream.ReadByte();
                            if(c == -1) break;
                            else if(c == Symbol.OpenRoundBracket) level++;
                            else if(c == Symbol.CloseRoundBracket) level--;
                            else if(c == '\\') {
                                bool lineBreak = false;
                                c = stream.ReadByte();
                                switch(c) {
                                case 'n': c = Symbol.LineFeed; break;
                                case 'r': c = Symbol.CarriageReturn; break;
                                case 't': c = '\t'; break;
                                case 'b': c = '\b'; break;
                                case 'f': c = '\f'; break;
                                ...
                                case Symbol.CarriageReturn:
                                  lineBreak = true;
                                  c = stream.ReadByte();
                                  if(c != Symbol.LineFeed) stream.Skip(-1);
                                  break;
                        ...
                        if(c == -1) throw new ParseException("Malformed literal string.");
                    } break;
