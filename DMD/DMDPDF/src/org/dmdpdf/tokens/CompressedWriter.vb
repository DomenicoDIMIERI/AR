...
#Region "Protected"
        ...
        Protected Overrides Sub WriteIncremental()
            ...
            '  a New section.
            '*/
            Dim xrefStream As XRefStream = New XRefStream(_file)
            ...
            For Each indirectObject As PdfIndirectObject In New List(Of PdfIndirectObject)(indirectObjects.ModifiedObjects.Values)
                prevFreeEntry = AddXRefEntry(indirectObject.XrefEntry,
                                                indirectObject,
                                                xrefStream,
                                                prevFreeEntry,
            ...
            Dim tmp As New PdfIndirectObject(
                                      _file,
                                      xrefStream,
                                      xrefStreamEntry
                                      )
            UpdateTrailer(xrefStream.Header, _stream)
...
#Region "private"
        ...
        '*/
        Private Function AddXRefEntry(
                                  ByVal xRefEntry As XRefEntry,
                                  ByVal indirectObject As PdfIndirectObject,
                                  ByVal xrefStream As XRefStream,
                                  ByVal prevFreeEntry As XRefEntry,
                                  ByVal extensionObjectStreams As IDictionary(Of Integer, ObjectStream)
                                  ) As XRefEntry
            xrefStream(xRefEntry.Number) = xRefEntry

            Select Case (xRefEntry.Usage)
                Case XRefEntry.UsageEnum.InUse
                    Dim offset As Integer = CInt(_stream.Length)
                    ' Add entry content!
          ...
          'break;
                Case XRefEntry.UsageEnum.InUseCompressed
                    ...
                    '*/
                    If (extensionObjectStreams IsNot Nothing) Then ' Incremental Then update.
                        Dim baseStreamNumber As Integer = xRefEntry.StreamNumber
                        Dim baseStreamIndirectObject As PdfIndirectObject = _file.IndirectObjects(baseStreamNumber)
                        If (baseStreamIndirectObject.IsOriginal()) Then ' Extension Then stream needed In order To preserve the original Object stream.
                            ...
                            If (Not extensionObjectStreams.TryGetValue(baseStreamNumber, extensionObjectStream)) Then
                                ...
                                _file.Register(extensionObjectStream)
