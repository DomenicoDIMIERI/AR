...
namespace org.dmdpdf.tokens
{
  ...
  */
  public abstract class Writer
  {
    #region static
    #region fields
    private static readonly byte[] _BOFChunk = Encoding.Pdf.Encode(Keyword.BOF);
    private static readonly byte[] _EOFChunk = Encoding.Pdf.Encode(Symbol.LineFeed + Keyword.EOF + Symbol.CarriageReturn + Symbol.LineFeed);
    private static readonly byte[] _HeaderBinaryHintChunk = new byte[]{(byte)Symbol.LineFeed,(byte)Symbol.Percent,(byte)0x80,(byte)0x80,(byte)0x80,(byte)0x80,(byte)Symbol.LineFeed}; // NOTE: Arbitrary binary characters (code >= 128) for ensuring proper behavior of file transfer applications [PDF:1.6:3.4.1].
    private static readonly byte[] _StartXRefChunk = Encoding.Pdf.Encode(Keyword.StartXRef + Symbol.LineFeed);
    #endregion
