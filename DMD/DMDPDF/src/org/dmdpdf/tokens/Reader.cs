...
using org.dmdpdf.util.collections.generic;
using org.dmdpdf.util.parsers;
  ...
  */
    ...
    : IDisposable
  {
    ...
    public sealed class FileInfo
    {
      ...
      private SortedDictionary<int,XRefEntry> _xrefEntries;
    ...
    {this._parser = new FileParser(stream, file);}
    #endregion
  ...
  //TODO:hybrid xref table/stream
      ...
      SortedDictionary<int,XRefEntry> xrefEntries = new SortedDictionary<int,XRefEntry>();
      {
        ...
        while(sectionOffset > -1)
        {
          ...
          if(_parser.GetToken(1).Equals(Keyword.XRef)) // XRef-table section.
          {
            ...
            while(true)
            {
              ...
              */
              ...
              _parser.MoveNext();
              if((_parser.TokenType == PostScriptParser.TokenTypeEnum.Keyword)
                  && _parser.Token.Equals(Keyword.Trailer)) // XRef-table section ended.
                break;
              else if(_parser.TokenType != PostScriptParser.TokenTypeEnum.Integer)
                throw new ParseException("Neither object number of the first object in this xref subsection nor end of xref section found.",_parser.Position);

              // Get the object number of the first object in this xref-table subsection!
              int startObjectNumber = (int)_parser.Token;

              // 2. Last object number.
              _parser.MoveNext();
              if(_parser.TokenType != PostScriptParser.TokenTypeEnum.Integer)
                throw new ParseException("Number of entries in this xref subsection not found.", _parser.Position);

              // Get the object number of the last object in this xref-table subsection!
              int endObjectNumber = (int)_parser.Token + startObjectNumber;
                ...
                )
              {
                ...
                int offset = (int)_parser.GetToken(1);
                // Get the object generation number!
                int generation = (int)_parser.GetToken(1);
                // Get the usage tag!
                XRefEntry.UsageEnum usage;
                {
                  string usageToken = (string)_parser.GetToken(1);
            ...
            sectionTrailer = (PdfDictionary)_parser.ParsePdfObject(1);
          }
          else // XRef-stream section.
          {
            XRefStream stream = (XRefStream)_parser.ParsePdfObject(3); // Gets the xref stream skipping the indirect-object header.
