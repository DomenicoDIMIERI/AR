...
#Region "fields"
        ...
        Private Shared ReadOnly TrailerChunk As Byte() = Encoding.Pdf.Encode(Keyword.Trailer & Symbol.LineFeed)
        Private Shared ReadOnly XRefChunk As String = Keyword.XRef & Symbol.LineFeed
        Private Shared ReadOnly XRefEOLChunk As String = "" & Symbol.CarriageReturn & Symbol.LineFeed

        Private Const XRefGenerationFormat As String = "00000"
        Private Const XRefOffsetFormat As String = "0000000000"
...
#Region "Protected"
        ...
        Protected Overrides Sub WriteIncremental()
            ...
            _stream.Write(parser.Stream)

            ' 2. Body update (modified indirect objects insertion).
            Dim xrefSize As Integer = _file.IndirectObjects.Count
            Dim xrefBuilder As StringBuilder = New StringBuilder(XRefChunk)
            '{
            '/*
            ...
            For Each indirectObjectEntry As KeyValuePair(Of Integer, PdfIndirectObject) In _file.IndirectObjects.ModifiedObjects
                ...
                If (indirectObjectEntry.Value.IsInUse()) Then ' In-use entry.
                    ...
                    indirectObjectEntry.Value.WriteTo(_stream, _file)
                Else ' Free entry.
                    '// Add free entry!
                    '/*
            ...
            Dim startxref As Long = _stream.Length
            _stream.Write(xrefBuilder.ToString())
        ...
        Protected Overrides Sub WriteStandard()
            ...
            Dim indirectObjects As IndirectObjects = _file.IndirectObjects
            ...
            For index As Integer = 1 To xrefSize - 1
                ...
                If (indirectObject.IsInUse()) Then ' In-use entry.
                    ...
                    ' Add in-use entry content!
                    indirectObject.WriteTo(_stream, _file)
                Else ' Free entry.
                    ' Add free entry!
                    AppendXRefEntry(
                              xrefBuilder,
            ...
            ' 3. XRef table (unique section) [PDF1.63.4.3].
            Dim startxref As Long = _stream.Length
            _stream.Write(xrefBuilder.ToString())

            ' 4. Trailer [PDF1.63.4.4].
...
#Region "Private"
        ...
        Private Function AppendXRefEntry(ByVal xrefBuilder As StringBuilder, ByVal reference As PdfReference, ByVal offset As Long) As StringBuilder
            ...
            Return xrefBuilder.Append(offset.ToString(XRefOffsetFormat)).Append(Symbol.Space).Append(reference.GenerationNumber.ToString(XRefGenerationFormat)).Append(Symbol.Space).Append(usage).Append(XRefEOLChunk)
        End Function
        ...
        Private Sub WriteTrailer(ByVal startxref As Long, ByVal xrefSize As Integer, ByVal parser As FileParser)
            ' 1. Header.
            _stream.Write(TrailerChunk)

            ' 2. Body.
            ' Update its entries:
            Dim trailer As PdfDictionary = _file.Trailer
            UpdateTrailer(trailer, _stream)
            ' * Size
            ...
            trailer.WriteTo(_stream, _file) : _stream.Write(Chunk.LineFeed)
