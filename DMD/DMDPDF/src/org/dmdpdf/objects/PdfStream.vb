...
#Region "constructors"
        ...
        Public Sub New(ByVal header As PdfDictionary)
            Me.New(header, New bytes.Buffer())
        End Sub

        Public Sub New(ByVal body As IBuffer)
            Me.New(New PdfDictionary(), body)
        End Sub
        ...
        Public Sub New(ByVal header As PdfDictionary, ByVal body As IBuffer)
            ...
            Me._body = body
            body.Dirty = False
            AddHandler body.OnChange, AddressOf Me.hHandler
        End Sub

        Private Sub hHandler(ByVal sender As Object, ByVal args As System.EventArgs)
            Me.Update()
        End Sub
...
#Region "Public"
        ...
        Public Property Filter As PdfDirectObject
            Get
                If (_header(PdfName.F) Is Nothing) Then
                    Return CType(_header.Resolve(PdfName.Filter), PdfDirectObject)
                Else
            ...
            Protected Set(ByVal value As PdfDirectObject)
                If (_header(PdfName.F) Is Nothing) Then
                    _header(PdfName.Filter) = value
                Else
                    _header(PdfName.FFilter) = value
                End If
            End Set
        End Property
        ...
        Public Function GetBody(ByVal decode As Boolean) As IBuffer
            If (Not _bodyResolved) Then
                ...
                If (dataFile IsNot Nothing) Then
                    Updateable = False
                    _body.SetLength(0)
                    _body.Write(dataFile.GetInputStream())
                    _body.Dirty = False
                    Updateable = True
                End If
                _bodyResolved = True
            ...
            If (decode) Then
                ...
                If (filter IsNot Nothing) Then ' Stream Then encoded.
                    _header.Updateable = False
                    Dim parameters As PdfDataObject = Me.parameters
                    If (TypeOf (filter) Is PdfName) Then ' Single Then filter.
                        _body.Decode(bytes.filters.Filter.Get(CType(filter, PdfName)), CType(parameters, PdfDictionary))
                    Else ' Multiple filters.
                        ...
                        While (filterIterator.MoveNext())
                            ...
                            End If
                            _body.Decode(bytes.filters.Filter.Get(CType(Resolve(filterIterator.Current), PdfName)), filterParameters)
                        End While
                    End If
                    Me.Filter = Nothing  ' The stream Is free from encodings.
                    _header.Updateable = True
                End If
            End If
            Return _body
        End Function
        ...
        Public Property Parameters As PdfDirectObject
            Get
                If (_header(PdfName.F) Is Nothing) Then
                    Return CType(_header.Resolve(PdfName.DecodeParms), PdfDirectObject)
                Else
                    Return CType(_header.Resolve(PdfName.FDecodeParms), PdfDirectObject)
                End If
            End Get
            Protected Set(ByVal value As PdfDirectObject)
                If (_header(PdfName.F) Is Nothing) Then
                    _header(PdfName.DecodeParms) = value
                Else
                    _header(PdfName.FDecodeParms) = value
                End If
            End Set
        End Property
            ...
            Else
                If (oldDataFile IsNot Nothing) Then
                    If (preserve) Then ' Case G (import old file to local).
                        ...
                        _header(PdfName.Filter) = _header(PdfName.FFilter) : _header.Remove(PdfName.FFilter)
                        _header(PdfName.DecodeParms) = _header(PdfName.FDecodeParms) : _header.Remove(PdfName.FDecodeParms)
                ...
                End If
            End If
            _header(PdfName.F) = dataFileObject
        End Sub
        ...
        Public Overrides Sub WriteTo(ByVal stream As IOutputStream, ByVal context As File)
            ...
            '*/
            Dim encodeBody As Boolean = (dataFile Is Nothing OrElse (_bodyResolved AndAlso _body.Dirty))
            If (encodeBody) Then
                Dim filterObject As PdfDirectObject = Me.Filter
                If (filterObject Is Nothing) Then '// UnencodedThen body.
                    '/*
                    ...
                    bodyData = _body.Encode(bytes.filters.Filter.Get(CType(filterObject, PdfName)), Nothing)
                    ' Set 'Filter' entry!
                ...
                If (dataFile IsNot Nothing) Then
                    ...
                    Catch e As System.Exception
                        Throw New System.Exception("Data writing into " & dataFile.Path & " failed.", e)
                    End Try
            ...
            _header(PdfName.Length) = PdfInteger.Get(bodyData.Length)

            ' 1. Header.
            _header.WriteTo(stream, context)

            If (bodyUnencoded) Then
                ' Restore actual header entries!
                _header(PdfName.Length) = PdfInteger.Get(CInt(_body.Length))
                Me.Filter = Nothing
            End If
            '}
...
#Region "IFileResource"
        ...
        Public Property DataFile As FileSpecification Implements IFileResource.DataFile
            Get
                Return FileSpecification.Wrap(_header(PdfName.F))
            End Get
...
'            '  Old data source | New data source | preserve | Action
'            '  ----------------------------------------------------------------------------------------------
'            '  local           | Not Nothing        | false     | A. Substitute local with New file.
'            '  local           | Not Nothing        | true      | B. Export local to New file.
'            '  external        | Not Nothing        | false     | C. Substitute old file with New file.
'            '  external        | Not Nothing        | true      | D. Copy old file data to New file.
'            '  local           | Nothing            | (any)     | E. No action.
'            '  external        | Nothing            | false     | F. Empty local.
'            '  external        | Nothing            | true      | G. Import old file to local.
'            '  ----------------------------------------------------------------------------------------------
'            '*/
'            Dim oldDataFile As FileSpecification = Me.dataFile
'            Dim dataFileObject As PdfDirectObject
