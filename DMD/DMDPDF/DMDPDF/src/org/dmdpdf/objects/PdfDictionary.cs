...
namespace org.dmdpdf.objects
{
  ...
  */
      ...
      IDictionary<PdfName,PdfDirectObject>
  {
    #region static
    #region fields
    private static readonly byte[] _BeginDictionaryChunk = Encoding.Pdf.Encode(Keyword.BeginDictionary);
    private static readonly byte[] _EndDictionaryChunk = Encoding.Pdf.Encode(Keyword.EndDictionary);
    #endregion
    #endregion

    #region dynamic
    #region fields
    internal IDictionary<PdfName,PdfDirectObject> _entries;
    ...
    {_entries = new Dictionary<PdfName,PdfDirectObject>();}

    public PdfDictionary(
      int capacity
      )
    {_entries = new Dictionary<PdfName,PdfDirectObject>(capacity);}
      ...
      )
    {
      return base.Equals(@object)
        || (@object != null
          ...
          && ((PdfDictionary)@object)._entries.Equals(_entries));
    }
    ...
    */
      ...
      )
    {
      ...
      */
      foreach(KeyValuePair<PdfName,PdfDirectObject> entry in _entries)
      {
    ...
    */
      ...
      )
    {
      ...
      IDictionary<PdfName,PdfDirectObject> otherEntries = otherDictionary._entries;
      // Update the other!
      otherDictionary._entries = this._entries;
      otherDictionary.Update();
      // Update this one!
      this._entries = otherEntries;
      this.Update();
      return this;
    }
    ...
    {
      _entries.Add(key, (PdfDirectObject)Include(value));
      Update();
    }
      ...
      ]
    {
      ...
      set
      {
        ...
        {
          PdfDirectObject oldValue = this[key];
          _entries[key] = (PdfDirectObject)Include(value);
          Exclude(oldValue);
          Update();
        }
      }
    ...
    {
      foreach(PdfName key in new List<PdfDirectObject>(_entries.Keys))
      {Remove(key);}
    }
    ...
    {return ((ICollection<KeyValuePair<PdfName,PdfDirectObject>>)_entries).Contains(entry);}
