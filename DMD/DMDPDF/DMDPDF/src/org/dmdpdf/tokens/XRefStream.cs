...
namespace org.dmdpdf.tokens
{
  ...
  */
      ...
      IDictionary<int,XRefEntry>
  {
    #region static
    #region fields
    private const int _FreeEntryType = 0;
    private const int _InUseEntryType = 1;
    private const int _InUseCompressedEntryType = 2;

    private static readonly double _ByteBaseLog = Math.Log(256);

    private static readonly int _EntryField0Size = 1;
    private static readonly int _EntryField2Size = GetFieldSize(XRefEntry.GenerationUnreusable);
    #endregion
    ...
    */
    ...
    {return (int)Math.Ceiling(Math.Log(maxValue)/_ByteBaseLog);}
    ...
    #region dynamic
    #region fields
    private SortedDictionary<int,XRefEntry> _entries;
    #endregion
    ...
    */
    ...
    {
      if(_entries == null)
      {_entries = new SortedDictionary<int,XRefEntry>();}
      else
      {_entries.Clear();}
    }
                  ...
                  )
                {
                  ...
                  switch(entryFieldType)
                  {
                    case _FreeEntryType:
                    {
                      ...
                      _entries[entryIndex] = new XRefEntry(
                        entryIndex,
                        generation,
    ...
    */
    private void Flush( IOutputStream stream )
    {
        ...
        };
      {
        ...
        foreach(XRefEntry entry in _entries.Values)
        {
          ...
          switch(entry.Usage)
          {
            case XRefEntry.UsageEnum.Free:
              body.Append((byte)_FreeEntryType);
            ...
            case XRefEntry.UsageEnum.InUse:
              body.Append((byte)_InUseEntryType);
              body.Append(NumberToByteArray(entry.Offset, entryFieldSizes[1]));
              body.Append(NumberToByteArray(entry.Generation, entryFieldSizes[2]));
              break;
            case XRefEntry.UsageEnum.InUseCompressed:
              body.Append((byte)_InUseCompressedEntryType);
