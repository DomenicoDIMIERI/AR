...
namespace org.dmdpdf.documents.contents.fonts
{
  /**
    ...
    : PdfObjectWrapper<PdfDictionary>
  {
    ...
    */
    public static Font Wrap( PdfDirectObject baseObject ) {
      ...
      if(fontType == null) throw new Exception("Font type undefined (reference: " + reference + ")");
    ...
    */
      ...
      )
    {
      ...
      {
        byte[][] codeBuffers = new byte[_charCodeMaxLength+1][];
        for(
          int codeBufferIndex = 0;
          codeBufferIndex <= _charCodeMaxLength;
          codeBufferIndex++
          )
        ...
        while(position < codeLength)
        {
          ...
          int textChar;
          if(!_codes.TryGetValue(new ByteArray(codeBuffer),out textChar))
          {
            if(codeBufferSize < _charCodeMaxLength)
            {
              codeBufferSize++;
              continue;
            }
    ...
    */
      ...
      )
    {
      ...
      for(int index = 0, length = text.Length; index < length; index++)
      {
        ...
        byte[] charCode = _codes.GetKey(textCode).Data;
    ...
    */
    ...
    {
      if(_glyphKernings == null)
        return 0;

      int textChar1Index;
      if(!_glyphIndexes.TryGetValue((int)textChar1, out textChar1Index))
        return 0;

      int textChar2Index;
      if(!_glyphIndexes.TryGetValue((int)textChar2, out textChar2Index))
        return 0;
    ...
    */
    ...
    {
      int glyphIndex;
      if(!_glyphIndexes.TryGetValue((int)textChar, out glyphIndex))
        return 0;

      int glyphWidth;
      return _glyphWidths.TryGetValue(glyphIndex, out glyphWidth) ? glyphWidth : _defaultGlyphWidth;
    }
    ...
    */
      ...
      )
    {
      if(BaseDataObject.ContainsKey(PdfName.ToUnicode)) // To-Unicode explicit mapping.
      {
        ...
        _codes = new BiDictionary<ByteArray,int>(parser.Parse());
        _symbolic = false;
      }
      ...
      foreach(ByteArray charCode in _codes.Keys)
      {
        if(charCode.Data.Length > _charCodeMaxLength)
