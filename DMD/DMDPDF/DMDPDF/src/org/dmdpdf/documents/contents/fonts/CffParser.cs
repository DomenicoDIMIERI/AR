  ...
  */
  internal sealed class CffParser
  {
    ...
    */
    ...
    private readonly IInputStream _fontData;
    private Index _stringIndex;
    #endregion
    ...
    */
      ...
      )
    {
      ...
      {
        ParseHeader();
        Index nameIndex = Index.Parse(_fontData);
        Index topDictIndex = Index.Parse(_fontData);
        _stringIndex = Index.Parse(_fontData);
        #pragma warning disable 0219
        Index globalSubrIndex = Index.Parse(_fontData);
  ...
  //      int encodingOffset = topDict.get(Dict.OperatorEnum.Encoding, 0, 0).intValue();
        ...
        Index charStringsIndex = Index.Parse(_fontData, charStringsOffset);
        ...
        else
        {
          ...
          int charsetFormat = _fontData.ReadByte();
          for (int index = 1, count = charStringsIndex.Count; index <= count;)
          {
            switch(charsetFormat)
            {
              case 0:
                glyphIndexes[index++] = ToUnicode(_fontData.ReadUnsignedShort());
                break;
              case 1:
              case 2:
              {
                int first = _fontData.ReadUnsignedShort();
                int nLeft = (charsetFormat == 1 ? _fontData.ReadByte() : _fontData.ReadUnsignedShort());
    ...
    */
      ...
      )
    {
      return id < StandardStrings.Count
        ? StandardStrings[id]
        : ToString(_stringIndex[id - StandardStrings.Count]);
    }
    ...
    {
      _fontData.Seek(2);
      int hdrSize = _fontData.ReadByte();
