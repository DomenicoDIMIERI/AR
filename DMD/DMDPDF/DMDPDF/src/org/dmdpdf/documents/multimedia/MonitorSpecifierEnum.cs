...
namespace org.dmdpdf.documents.multimedia
{
  ...
  */
  ...
  {
    private static readonly BiDictionary<MonitorSpecifierEnum,PdfInteger> _codes;

    static MonitorSpecifierEnumExtension()
    {
      _codes = new BiDictionary<MonitorSpecifierEnum,PdfInteger>();
      _codes[MonitorSpecifierEnum.LargestDocumentWindowSection] = new PdfInteger(0);
      _codes[MonitorSpecifierEnum.SmallestDocumentWindowSection] = new PdfInteger(1);
      _codes[MonitorSpecifierEnum.Primary] = new PdfInteger(2);
      _codes[MonitorSpecifierEnum.GreatestColorDepth] = new PdfInteger(3);
      _codes[MonitorSpecifierEnum.GreatestArea] = new PdfInteger(4);
      _codes[MonitorSpecifierEnum.GreatestHeight] = new PdfInteger(5);
      _codes[MonitorSpecifierEnum.GreatestWidth] = new PdfInteger(6);
    }

    public static MonitorSpecifierEnum? Get( PdfInteger code )
    {
      if(code == null)
        return MonitorSpecifierEnum.LargestDocumentWindowSection;

      MonitorSpecifierEnum? monitorSpecifier = _codes.GetKey(code);
    ...
    {return _codes[monitorSpecifier];}
  }
}