...
public class Type1CharStringParser
{
     ...
     */
    public List(Of Object) parse(byte[] bytes, IndexData localSubrIndex) throws IOException
    {
        return parse(bytes, localSubrIndex, true);
    }
    
    private List(Of Object) parse(byte[] bytes, IndexData localSubrIndex, boolean init) throws IOException
    {
        if (init) 
        {
