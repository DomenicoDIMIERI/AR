...
public class CMAPEncodingEntry
{
	...
	private Map(Of NInteger, NInteger) characterCodeToGlyphId = new HashMap(Of NInteger, NInteger)();
	...
	protected void processSubtype8( TrueTypeFont ttf, TTFDataStream data, int numGlyphs ) 
		...
		for (long i = 0; i <= nbGroups ; ++i ) 
		{
			...
			for (long j = firstCode; j <= endCode; ++j) {
				...
				if (glyphIndex > numGlyphs || glyphIndex > Integer.MaxValue) {
	...
	protected void processSubtype12( TrueTypeFont ttf, TTFDataStream data, int numGlyphs ) 
		...
		for (long i = 0; i <= nbGroups ; ++i ) 
		{
			...
			for (long j = 0; j <= (endCode - firstCode); ++j) {
				...
				if ( (firstCode + j) > Integer.MaxValue ) {
	...
	protected void processSubtype4( TrueTypeFont ttf, TTFDataStream data, int numGlyphs ) 
		...
		Map(Of NInteger, NInteger) tmpGlyphToChar = new HashMap(Of NInteger, NInteger)();
		 ...
		 */
		glyphIdToCharacterCode = Arrays.CreateInstance(Of Integer)(Collections.max(tmpGlyphToChar.keySet())+1];
		Arrays.fill(glyphIdToCharacterCode, 0);
		for (Entry(Of NInteger, NInteger) entry : tmpGlyphToChar.entrySet()) {
	...
	protected void processSubtype2( TrueTypeFont ttf, TTFDataStream data, int numGlyphs ) 
			throws IOException {
		int[] subHeaderKeys = Arrays.CreateInstance(Of Integer)(256];
	...
	public int getGlyphId(int characterCode) {
		if (Me.characterCodeToGlyphId.containsKey(characterCode)) 
		{
