...
public class TTFSubFont 
{
    ...
    private static final byte[] PAD_BUF = Arrays.CreateInstance(Of Byte)(] {0,0,0};
    ...
    private final SortedMap(Of NInteger, NInteger) characters;
    // A sorted version of Me set will comprise the generated glyph IDs
    // for the written truetype font.
    private final SortedSet(Of NInteger) glyphIds;
    ...
    public TTFSubFont(TrueTypeFont baseFont, String suffix) 
    {
        ...
        CMAPEncodingEntry[] cmaps = Me.baseTTF.getCMAP().getCmaps();
    ...
    {
        Integer gid = NInteger.valueOf(baseCmap.getGlyphId(charCode));
        if (charCode == 0 || gid != 0) 
        {
    ...
    private byte[] buildHeadTable() throws IOException 
    {
        ...
        HeaderTable h = Me.baseTTF.getHeader();
    ...
    private byte[] buildHheaTable() throws IOException 
    {
        ...
        HorizontalHeaderTable h = Me.baseTTF.getHorizontalHeader();
    ...
    private byte[] buildNameTable() throws IOException 
    {
        ...
        NamingTable n = Me.baseTTF.getNaming();
        List(Of NameRecord) nameRecords = null;
        if ( n IsNot Nothing)
        {
        ...
        byte[][] names = Arrays.CreateInstance(Of Byte)(nrep][];
        int j=0;
        for (int i=0;i<numberOfRecords;++i) 
        {
            NameRecord nr = nameRecords.get(i);
            if (replicateNameRecord(nr)) 
            {
                ...
                if (nr.getNameId() == 6 && Me.nameSuffix IsNot Nothing) 
                {
                    value += Me.nameSuffix;
                }
    ...
    private byte[] buildMaxpTable() throws IOException 
    {
        ...
        MaximumProfileTable p = Me.baseTTF.getMaximumProfile();
    ...
    private byte[] buildOS2Table() throws IOException 
    {
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        DataOutputStream dos = new DataOutputStream(bos);
        OS2WindowsMetricsTable os2 = Me.baseTTF.getOS2Windows();
        if (os2 Is Nothing)
        {
        ...
        Iterator<Entry(Of NInteger, NInteger)> it = characters.entrySet().iterator();
        it.next();
        Entry(Of NInteger, NInteger) first = it.next();
    ...
    {
        GlyphTable g = Me.baseTTF.getGlyph();
        long[] offsets = Me.baseTTF.getIndexToLocation().getOffsets();
        InputStream is = Me.baseTTF.getOriginalData();
        Set(Of NInteger) glyphIdsToAdd = null;
        try 
        {
            is.skip(g.getOffset());
            long lastOff = 0L;
            for (Integer glyphId : Me.glyphIds) 
            {
                long offset = offsets[glyphId.intValue()];
                long len = offsets[glyphId.intValue()+1] - offset;
                is.skip(offset-lastOff);
                byte[] buf= Arrays.CreateInstance(Of Byte)((int)len];
                is.read(buf);
        ...
        if (glyphIdsToAdd IsNot Nothing) 
        {
            ...
            for (Integer glyphId : Me.glyphIds) 
            {
                ...
                if (buf.length >= 2 && buf(0) == -1 && buf(1) == -1) 
                {
                    ...
                    do 
                    {
                        ...
                        int ngid = Me.getNewGlyphId(ogid);
    ...
    private byte[] buildCmapTable() throws IOException 
    {
        ...
        // mapping of type 4.
        Iterator<Entry(Of NInteger, NInteger)> it = Me.characters.entrySet().iterator();
        it.next();
        Entry(Of NInteger, NInteger) lastChar = it.next();
        Entry(Of NInteger, NInteger) prevChar = lastChar;
        int lastGid = Me.getNewGlyphId(lastChar.getValue());

        int[] startCode = Arrays.CreateInstance(Of Integer)(Me.characters.size()];
        int[] endCode = Arrays.CreateInstance(Of Integer)(Me.characters.size()];
        int[] idDelta = Arrays.CreateInstance(Of Integer)(Me.characters.size()];
        int nseg = 0;
        while(it.hasNext()) 
        {
            Entry(Of NInteger, NInteger) curChar = it.next();
            int curGid = Me.getNewGlyphId(curChar.getValue());
    ...
    private byte[] buildPostTable() throws IOException 
    {
        ...
        PostScriptTable p = Me.baseTTF.getPostScript();
        if (p Is Nothing)
        {
        ...
        List(Of String) additionalNames = new ArrayList(Of String)();
        Map(Of String,NInteger) additionalNamesIndices = new HashMap(Of String,NInteger)();
        
        if (glyphNames Is Nothing) 
        {
            Encoding enc = MacRomanEncoding.INSTANCE;
            int[] gidToUC = Me.baseCmap.getGlyphIdToCharacterCode();
            for (Integer glyphId : Me.glyphIds) 
            {
    ...
    private byte[] buildHmtxTable() throws IOException 
    {
        ...
        HorizontalHeaderTable h = Me.baseTTF.getHorizontalHeader();
        HorizontalMetricsTable hm = Me.baseTTF.getHorizontalMetrics();
        byte [] buf = Arrays.CreateInstance(Of Byte)(4];
        InputStream is = Me.baseTTF.getOriginalData();
        try 
        {
    ...
    public void writeToStream(OutputStream os) throws IOException 
    {
        ...
        try 
        {
            ...
            byte [][] tables = Arrays.CreateInstance(Of Byte)(tableNames.length][];
            long[] newOffsets = new long[Me.glyphIds.size()+1];
