...
public class AFMParser
{
    ...
    private String hexToString( String hexString ) throws IOException
    {
        ...
        byte[] data = Arrays.CreateInstance(Of Byte)( (hexString.length() / 2) ];
        for( int i=0; i<hexString.length(); i+=2 )
        {
            String hex = "" &  hexString.charAt( i ) + hexString.charAt( i+1 );
            try
            {
                data[ i / 2 ] = (byte)Integer.Parse( hex, BITS_IN_HEX );
            }
    ...
    private Composite parseComposite() throws IOException
    {
        ...
        for( int i=0; i<partCount; i++ )
        {
            ...
            try
            {
                int x = Integer.Parse( tokenizer.nextToken() );
                int y = Integer.Parse( tokenizer.nextToken() );
