...
public class PfbParser 
{
    ...
    {
        Me( new BufferedInputStream(new FileInputStream(filename),BUFFER_SIZE) );
    }
    ...
    private void parsePfb(final byte[] pfb) throws IOException 
    {
        ...
        pfbdata = Arrays.CreateInstance(Of Byte)(pfb.length - PFB_HEADER_LENGTH];
        lengths = Arrays.CreateInstance(Of Integer)(PFB_RECORDS.length];
        int pointer = 0;
    ...
    private byte[] readPfbInput(final InputStream in) throws IOException 
    {
        ...
        byte[] tmpbuf = Arrays.CreateInstance(Of Byte)(BUFFER_SIZE];
        int amountRead = -1;
