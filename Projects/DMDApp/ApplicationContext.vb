...
Public Class ApplicationContext
    ...
    Public Property CurrentUser As Sistema.CUser Implements DMD.Sistema.IApplicationContext.CurrentUser
        Get
            Return DIALTPLib.Remote.CurrentUser
        End Get
        Set(value As Sistema.CUser)
            DIALTPLib.Remote.CurrentUser = value
        End Set
    End Property
    ...
    Public Property CurrentSession As Object Implements IApplicationContext.CurrentSession
        Get
            Return DIALTPLib.Remote.CurrentSession
        End Get
        Set(value As Object)
            DIALTPLib.Remote.CurrentSession = value
        End Set
    End Property
    ...
    Public Function IsUserLogged(user As CUser) As Boolean Implements IApplicationContext.IsUserLogged
        Return GetID(user) = GetID(DIALTPLib.Remote.CurrentUser)
    End Function
...
#End If
    ...
    Public Overridable Property IDAziendaPrincipale As Integer Implements IApplicationContext.IDAziendaPrincipale
        Get
            Return GetID(DIALTPLib.Remote.AziendaPrincipale)
        End Get
    ...
    Public Sub Start() Implements IApplicationContext.Start
        AddHandler Sistema.Events.UnhandledException, AddressOf handleUnhException
        'AddHandler DIALTPLib.Keyboard.KeyPressed, AddressOf handleKeyPress
        Try
            ...
            Dim s As New DIALTPLib.CLogSession
            s.EnsureFolder()
            Try

                Dim dbPath As String = System.IO.Path.Combine(Me.UserDataFolder, "db.mdb")
                'Dim dbPath As String = System.IO.Path.Combine(Me.StartupFloder, "db.mdb")
                If Not System.IO.File.Exists(dbPath) Then
                    Dim srcPath As String = System.IO.Path.Combine(Me.StartupFloder, "templates\db.mdb")
                    DIALTPLib.Log.LogMessage("Copio il db da [" & srcPath & "] a [" & dbPath & "]")
                    System.IO.File.Copy(srcPath, dbPath)
                End If
                DIALTPLib.Log.LogMessage("Apro il db [" & dbPath & "]")
                Dim dbConn As New CMdbDBConnection(dbPath)
                dbConn.OpenDB()
                Dim tbl As CDBTable = dbConn.Tables.GetItemByKey("tbl_Telefonate")
                If (Not tbl.Fields.ContainsKey("IDTelefonata")) Then
                    DIALTPLib.Log.LogMessage("Aggiungo la colonna [IDTelefonata] alla tabella [tbl_Telefonate]")
                    Dim col As CDBEntityField = tbl.Fields.Add("IDTelefonata", GetType(Integer))
                    tbl.Update()
                End If
                DIALTPLib.Chiamate.Database = dbConn
            Catch ex As Exception
                DIALTPLib.Log.LogException(ex)
            End Try

            DIALTPLib.Log.LogMessage("Inizializzo gli handlers")
            ...
            With DirectCast(faxDriver.Config, HylafaxDriverConfiguration)
            ...
            DIALTPLib.Log.LogMessage("Preparo la cartella [" & path & "]")
            DMD.Sistema.FileSystem.CreateRecursiveFolder(path)
        ...
        Catch ex As Exception
            Me.Log("Errore di inizializzazione " & ex.Message & vbNewLine & ex.StackTrace)
            MsgBox(ex.Message, MsgBoxStyle.Critical, "Errore di Inizializzazione")
        End Try
    End Sub

    Private Sub handleUnhException(ByVal e As System.Exception)
        Try
            Me.Log("Eccezione non gestita: " & e.Message & vbNewLine & e.StackTrace)
        Catch ex As Exception
    ...
    Private Class MyNewTypeHandler
        Inherits NewTypeHandler

        Public Overrides Function FindType(typeName As String) As Type
            If (typeName = "DeviceLog") Then
                Return GetType(DMD.Office.DeviceLog)
            Else
                Return Nothing
            End If
        End Function
    ...
    Public Sub [Stop]() Implements IApplicationContext.Stop
        ...
        For Each faxDriver As DMD.Sistema.BaseFaxDriver In DMD.Sistema.FaxService.GetInstalledDrivers
            Try
                If (faxDriver.IsConnected) Then faxDriver.Disconnect()
            Catch ex As Exception
        ...
        Sistema.Types.NewTypeHandlers.Add(New MyNewTypeHandler)
    ...
    Public Sub Log(message As String) Implements IApplicationContext.Log
        DIALTPLib.Log.InternalLog(message)
    End Sub
