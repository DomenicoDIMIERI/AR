...
Namespace Internals
    ...
    Public NotInheritable Class CEMailerClass
        Private Class SendingMailInfo
            ...
            'Public mail As MailMessageEx = Nothing
...
#End If
        ...
        Public Function IsNewMessage(ByVal message As DMD.Net.Mail.MailMessageEx, Optional ByVal account As CEmailAccount = Nothing) As Boolean
            ...
            If (account Is Nothing) Then
                'Return ((Me.Config.LastSync.HasValue = False) OrElse (DateUtils.DateDiff(DateInterval.Minute, Me.Config.LastSync.Value, message.DeliveryDate) > -5)) AndAlso _
                '   Not Global.System.IO.File.Exists(Me.GetTargetName(message, account))
                Return Not Global.System.IO.File.Exists(Me.GetTargetName(message, account))
            Else
                'Return ((account.LastSync.HasValue = False) OrElse (DateUtils.DateDiff(DateInterval.Minute, account.LastSync.Value, message.DeliveryDate) > -5)) AndAlso _
                '    Not Global.System.IO.File.Exists(Me.GetTargetName(message, account))
                Return Not Global.System.IO.File.Exists(Me.GetTargetName(message, account))
            End If
        End Function
