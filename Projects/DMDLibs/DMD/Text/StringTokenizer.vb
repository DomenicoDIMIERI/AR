Namespace Text
    ...
    Public Class StringTokenizer
        ...
        '               Returns the next token in this string tokenizer's string. 
        '               First, the set of characters considered to be delimiters by this StringTokenizer object is changed to be 
        '               the characters in the string delim. Then the next token in the string after the current position is returned. 
        '               The current position is advanced beyond the recognized token. The new delimiter set remains the default after 
        '               this call.
        ...
        '           Returns the same value as the nextToken method, except that its declared return value is Object rather than 
        '           String. It exists so that this class can implement the Enumeration interface.
        ...
        Private m_ReturnDelimiters As Boolean
        Private m_CurrentPosition As Integer
        Private m_CurrentToken As System.Text.StringBuilder

        Sub New(ByVal [string] As String)
            Me.New([string], " ", False)
        End Sub

        Sub New(ByVal [string] As String, ByVal delimiters As String)
            Me.New([string], delimiters, False)
        End Sub

        Sub New(ByVal [string] As String, ByVal delimiters As String, ByVal returnDelimiters As Boolean)
            Me.m_String = [string]
            Me.m_Delimiters = delimiters
            Me.m_ReturnDelimiters = returnDelimiters
            Me.m_CurrentPosition = 0
            Me.m_CurrentToken = Nothing
        End Sub

        Function hasMoreTokens() As Boolean
            Return Me.m_CurrentPosition <= Len(Me.m_String)
        End Function

        Function nextToken() As String
            Me.m_CurrentToken = New System.Text.StringBuilder(Len(Me.m_String) + 1)
            Dim i As Integer = Me.m_CurrentPosition + 1
            While (i <= Len(Me.m_String))
                Dim ch As String = Mid(Me.m_String, i, 1)
                If (InStr(Me.m_Delimiters, ch) > 0) Then
                    If (Me.m_ReturnDelimiters) Then Me.m_CurrentToken.Append(ch)
                    Exit While
                End If
                Me.m_CurrentToken.Append(ch)
                i += 1
            End While
            Me.m_CurrentPosition = i
            Return Me.m_CurrentToken.ToString
        End Function

        Function countTokens() As Integer
            Dim cnt As Integer = 1
            For i = 1 To Len(Me.m_String)
                Dim ch As String = Mid(Me.m_String, i, 1)
                If (InStr(Me.m_Delimiters, ch) > 0) Then cnt += 1
            Next
            Return cnt
        End Function
