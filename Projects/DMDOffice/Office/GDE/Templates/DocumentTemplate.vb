...
Partial Class Office
    ...
    Public Class DocumentTemplate
        ...
        Public Sub RenderToPDFFile(ByVal context As Object, ByVal fileName As String)
            Dim srcPath As String = Me.m_SourceFile
            If (Strings.IndexOf(srcPath, "/") > 0) Then srcPath = Sistema.ApplicationContext.MapPath(srcPath)

            Dim srcFile As New File(srcPath)
            Dim srcDocument As Document = srcFile.Document

            Dim form As Form = srcDocument.Form
            If (Not form.Exists()) Then Throw New ArgumentNullException("Missing FORM in PDF")

            Dim maxIndex As Integer = 0

            Dim fields() As Field = form.Fields.Values.ToArray
            For Each field As Field In fields
                'Dim value As String
                If (TypeOf (field) Is RadioButton) Then
                    Dim radioField As RadioButton = field
                    'value = (field.Widgets(0)).WidgetName ' Selects the first widget in the group.
                ElseIf (TypeOf (field) Is ChoiceField) Then
                    Dim choiceField As ChoiceField = field
                ...
                End If
                field.Value = Formats.ToString(Me.GetFieldValueBase(context, field.FullName))
            Next

            srcFile.Save(fileName, SerializationModeEnum.Standard)
            srcFile.Dispose()
        End Sub

        Private Function GetFieldValue(ByVal context As Object, ByVal fieldName As String) As Object
            fieldName = Strings.Trim(fieldName)
            Dim oFieldName As String = fieldName
            Dim pDot As Integer = fieldName.IndexOf(".")
            Dim prop As System.Reflection.PropertyInfo
            Dim fName As String
            Dim fIndex As Integer? = Nothing
            Dim t As System.Type = context.GetType


            If (pDot > 0) Then
                fName = Strings.Trim(fieldName.Substring(0, pDot))
                fieldName = fieldName.Substring(pDot + 1)
            Else
                fName = fieldName
                fieldName = ""
            End If

            Dim pBraO As Integer = fName.IndexOf("(")
            If (pBraO >= 0) Then
                Dim pBraC As Integer = fName.IndexOf(")")
                If (pBraC <= pBraO) Then
                    Return "BAD FORMAT: " & oFieldName
                ElseIf (pBraC > pBrao) Then
                    fIndex = Int32.Parse(fName.Substring(pBraO + 1, pBraC - pBraO - 1))
                End If
                fName = fName.Substring(pBraO - 1)
            End If

            If (fIndex.HasValue) Then
                prop = t.GetProperty(fName, New System.Type() {GetType(Int32)})
            Else
                prop = t.GetProperty(fName)
            End If
            If (prop Is Nothing) Then Return "MISS. FIELD: " & fName

            Dim ret As Object
            If (fIndex.HasValue) Then
                ret = prop.GetValue(context, {fIndex.Value})
            Else
                ret = prop.GetValue(context, Nothing)
            End If

            If (fieldName <> "") Then
                Return Me.GetFieldValue(ret, fieldName)
            Else
                Return ret
            End If
        End Function

        Private Function GetFieldValueBase(ByVal context As Object, ByVal fieldName As String) As Object
            fieldName = Strings.Trim(fieldName)
            Dim oFieldName As String = fieldName
            Dim pDot As Integer = fieldName.IndexOf(".")
            Dim fName As String
            ...
            Select Case fName
                Case "Me"
                    Return Me.GetFieldValue(context, fieldName)
                Case "CurrentUser"
                    If (Sistema.Users.CurrentUser Is Nothing) Then Return ""
                    Return Me.GetFieldValue(Sistema.Users.CurrentUser, fieldName)
                Case "CurrentUfficio"
                    If (Anagrafica.Uffici.GetCurrentPOConsentito Is Nothing) Then Return ""
                    Return Me.GetFieldValue(Anagrafica.Uffici.GetCurrentPOConsentito, fieldName)
                Case "PrintDate" : Return Formats.FormatUserDate(DateUtils.Now)
                Case "PrintTime" : Return Formats.FormatUserDateTime(DateUtils.Now)
                Case Else
                    Return "MISS. FIELD: " & fName
            End Select
