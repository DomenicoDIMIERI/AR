...
Namespace Forms
    ...
    Public Class CAnagraficaModuleHandler
        ...
        Public Function GetRicontatti(ByVal renderer As Object) As String
            Dim pID As Nullable(Of Integer) = RPC.n2int(GetParameter(renderer, "pid", ""))
            If (pID.HasValue = False) Then Return ""
            'Dim nomeLista As String = RPC.n2str(GetParameter(renderer, "nl", ""))
            Dim ret As CCollection(Of CRicontatto) = Anagrafica.Ricontatti.GetRicontatti(pID) ', nomeLista
            Return XML.Utils.Serializer.Serialize(ret, XMLSerializeMethod.Document)
        End Function
        ...
        Public Function CheckRicontattiMultipli(ByVal renderer As Object) As String
            ...
            Try
                ...
                cursor.DataPrevista.Value = Calendar.ToDay
...
#End If
                        ...
                        Dim int As CIntervalloData = Calendar.PeriodoToDates(txtPer, txtDi, txtDf)
                        txtDi = int.Inizio : txtDf = int.Fine
                        If (txtDi.HasValue AndAlso txtDf.HasValue AndAlso txtDi.Value = txtDf.Value) Then txtDf = Calendar.DateAdd(DateInterval.Second, 24 * 3600 - 1, txtDi.Value)
