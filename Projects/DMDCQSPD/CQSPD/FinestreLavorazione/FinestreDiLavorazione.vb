...
Namespace Internals
    ...
    Public Class FinestreDiLavorazioneClass
        Inherits CGeneralClass(Of FinestraLavorazione)
        ...
        Public Function Ricostruisci(ByVal p As CPersonaFisica) As CCollection(Of FinestraLavorazione)
            Dim finestre As CCollection(Of FinestraLavorazione) = CQSPD.FinestreDiLavorazione.GetFinestreByPersona(p) : finestre.Sort()
            Dim pratiche As CCollection(Of CRapportino) = CQSPD.Pratiche.GetPraticheByPersona(p) : pratiche.Sort()
            Dim studif As CCollection(Of CQSPDConsulenza) = CQSPD.Consulenze.GetConsulenzeByPersona(p) : studif.Sort()
            Dim richiestef As CCollection(Of CRichiestaFinanziamento) = CQSPD.RichiesteFinanziamento.GetRichiesteByPersona(p) : richiestef.Sort()
            Dim contatti As CCollection(Of CContattoUtente) = CustomerCalls.CRM.GetContattiByPersona(p) : contatti.Sort()
            Dim prestiti As CCollection(Of CEstinzione) = CQSPD.Estinzioni.GetEstinzioniByPersona(p)
        ...
        Public Function HaCQS(ByVal persona As CPersonaFisica, ByVal al As Date?) As Boolean
            ...
            'If (al.HasValue = False) Then al = DateUtils.Now
            'For Each p As CCQSPDProdotto In CQSPD.Prodotti.LoadAll
