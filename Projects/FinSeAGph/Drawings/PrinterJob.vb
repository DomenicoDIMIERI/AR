Namespace Drawings
    ...
    Public Class PrinterJob
        ...
        'The PrinterJob class is the principal class that controls printing. An application calls methods in Me class to set up a job, optionally to invoke a print dialog with the user, and then to print the pages of the job.
        ...
        '    Calculates a PageFormat with values consistent with those supported by the current PrintService for Me job (ie the value returned by getPrintService()) and media, printable area and orientation contained in attributes.
        ...
        '    Returns the service (printer) for Me printer job.
        ...
        '    Associate Me PrinterJob with a new PrintService.
        '    abstract PageFormat 	validatePage(PageFormat page)
        '    Returns the clone of page with its settings adjusted to be compatible with the current printer of Me PrinterJob.
        ...
        '        Creates and returns a PrinterJob which is initially associated with the default printer. If no printers are available on the system, a PrinterJob will still be returned from Me method, but getPrintService() will return null, and calling print with Me PrinterJob might generate an exception. Applications that need to determine if there are suitable printers before creating a PrinterJob should ensure that the array returned from lookupPrintServices is not empty.

        '        Returns:
        '            a new PrinterJob.
        '        Throws:
        '            SecurityException - if a security manager exists and its SecurityManager.checkPrintJobAccess() method disallows Me thread from creating a print job request

        '        lookupPrintServices

        '        public static PrintService[] lookupPrintServices()

        '        A convenience method which looks up 2D print services. Services returned from Me method may be installed on PrinterJobs which support print services. Calling Me method is equivalent to calling PrintServiceLookup.lookupPrintServices() and specifying a Pageable DocFlavor.
        ...
        '        Services returned from Me method may be installed on PrinterJob instances which support print services. Calling Me method is equivalent to calling StreamPrintServiceFactory.lookupStreamPrintServiceFactories() and specifying a Pageable DocFlavor.
        ...
        '        Returns the service (printer) for Me printer job. Implementations of Me class which do not support print services may return null. null will also be returned if no printers are available.

        '        Returns:
        '            the service for Me printer job.
        '        Since:
        '            1.4
        '        See Also:
        ...
        '        Associate Me PrinterJob with a new PrintService. This method is overridden by subclasses which support specifying a Print Service. Throws PrinterException if the specified service cannot support the Pageable and Printable interfaces necessary to support 2D printing.

        '        Parameters:
        '            service - a print service that supports 2D printing
        '        Throws:
        '            PrinterException - if the specified service does not support 2D printing, or Me PrinterJob class does not support setting a 2D print service, or the specified service is otherwise not a valid print service.
        '        Since:
        '            1.4
        '        See Also:
        '            getPrintService()
        ...
        '        Calls painter to render the pages. The pages in the document to be printed by Me PrinterJob are rendered by the Printable object, painter. The PageFormat for each page is the default page format.
        ...
        '        Calls painter to render the pages in the specified format. The pages in the document to be printed by Me PrinterJob are rendered by the Printable object, painter. The PageFormat of each page is format.
        ...
        '        PrinterJob implementations which can use PrintService's will update the PrintService for Me PrinterJob to reflect the new service selected by the user.
        ...
        '        A convenience method which displays a cross-platform print dialog for all services which are capable of printing 2D graphics using the Pageable interface. The selected printer when the dialog is initially displayed will reflect the print service currently attached to Me print job. If the user changes the print service, the PrinterJob will be updated to reflect Me, unless the user cancels the dialog. As well as allowing the user to select the destination printer, the user can also select values of various print request attributes.
        ...
        '        Displays a dialog that allows modification of a PageFormat instance. The page argument is used to initialize controls in the page setup dialog. If the user cancels the dialog then Me method returns the original page object unmodified. If the user okays the dialog then Me method returns a new PageFormat object with the indicated changes. In either case, the original page object is not modified.
        ...
        '        A convenience method which displays a cross-platform page setup dialog. The choices available will reflect the print service currently set on Me PrinterJob.
        ...
        '        Calling Me method does not update the job. It is useful for clients that have a set of attributes obtained from printDialog(PrintRequestAttributeSet attributes) and need a PageFormat to print a Pageable object.
        ...
        '        Returns the clone of page with its settings adjusted to be compatible with the current printer of Me PrinterJob. For example, the returned PageFormat could have its imageable area adjusted to fit within the physical area of the paper that is used by the current printer.

        '        Parameters:
        '            page - the PageFormat that is cloned and whose settings are changed to be compatible with the current printer
        '        Returns:
        '            a PageFormat that is cloned from page and whose settings are changed to conform with Me PrinterJob.
        ...
        '        If a supported attribute-value is specified in Me attribute set, it will take precedence over the API settings for Me print() operation only. The following behaviour is specified for PageFormat: If a client uses the Printable interface, then the attributes parameter to Me method is examined for attributes which specify media (by size), orientation, and imageable area, and those are used to construct a new PageFormat which is passed to the Printable object's print() method. See Printable for an explanation of the required behaviour of a Printable to ensure optimal printing via PrinterJob. For clients of the Pageable interface, the PageFormat will always be as supplied by that interface, on a per page basis.

        '        These behaviours allow an application to directly pass the user settings returned from printDialog(PrintRequestAttributeSet attributes to Me print() method.
        ...
        '        Cancels a print job that is in progress. If print has been called but has not returned then Me method signals that the job should be cancelled at the next chance. If there is no print job in progress then Me call does nothing.
        '        isCancelled
        ...
        Shared Function getPrinterJob() As PrinterJob
            Throw New NotImplementedException
        End Function

        Function defaultPage() As PageFormat
            Throw New NotImplementedException
        End Function

        Sub setPageable(ByVal pDPageable As Object)
            Throw New NotImplementedException
        End Sub

        Function printDialog() As Boolean
            Throw New NotImplementedException
        End Function
