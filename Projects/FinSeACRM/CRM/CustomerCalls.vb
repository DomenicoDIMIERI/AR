...
Namespace Internals
    ...
    Public NotInheritable Class CCRMClass
        ...
        Public ReadOnly Property Index As CIndexingService
            Get
                SyncLock Me
                    If (Me.m_Index Is Nothing) Then Me.m_Index = New CIndexingService(Me.StatsDB)
        ...
        Private Sub handleNuovoContatto(ByVal e As CustomerCalls.ContattoEventArgs)
            ...
            Dim data As Date = Calendar.GetDatePart(e.Contatto.Data)
        ...
        '    data = Calendar.GetDatePart(data)
        '    Dim toDate As Date = Calendar.DateAdd(DateInterval.Day, 1, data)
        ...
        Public Function GetStoricoContatti(ByVal filter As CRMFindFilter) As CCollection(Of StoricoAction)
            ...
            If (filter.Dal.HasValue) Then filter.Dal = Calendar.GetDatePart(filter.Dal)
            If (filter.Al.HasValue) Then filter.Al = Calendar.DateAdd(DateInterval.Second, 24 * 3600 - 1, Calendar.GetDatePart(filter.Al))

            For Each h As IStoricoHandler In Me.Handlers
                Try
                    Dim t1 As Date = Calendar.Now
                    h.Aggiungi(items, filter)
                    Dim t2 As Date = Calendar.Now
                    ...
                    Case "anno"
                        ...
                        While dbRis.Read
                            ...
                            If (item Is Nothing) Then
                                ...
                                item.Data = Calendar.MakeDate(anno, 1, 1)
                                tmp.Add(key, item)
                            End If
                    ...
                    Case "mese"
                        ...
                        While dbRis.Read
                            ...
                            If (item Is Nothing) Then
                                ...
                                item.IDPersona = pid
                                item.NomePersona = nome
                                item.Data = Calendar.MakeDate(anno, mese, 1)
                                tmp.Add(key, item)
                            End If
                            If (esito = 1) Then item.ConteggioRisposte += cnt
                            item.ConteggioChiamate += cnt
                    ...
                    Case "giorno"
                        ...
                        While dbRis.Read
                            ...
                            If (item Is Nothing) Then
                                ...
                                item.IDPersona = pid
                                item.NomePersona = nome
                                item.Data = Calendar.MakeDate(anno, mese, giorno)
                                tmp.Add(key, item)
                            End If
                            If (esito = 1) Then item.ConteggioRisposte += cnt
                            item.ConteggioChiamate += cnt
        ...
        Public Sub RicalcolaStatistiche(ByVal data As Date)
            ...
            Try
                data = Calendar.GetDatePart(data)
                Dim toDate As Date = Calendar.DateAdd(DateInterval.Second, 3600 * 24 - 1, data)
