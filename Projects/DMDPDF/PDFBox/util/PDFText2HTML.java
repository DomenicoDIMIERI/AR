...
public class PDFText2HTML extends PDFTextStripper
{
    ...
    protected void writeHeader() throws IOException
    {
        ...
        if(outputEncoding IsNot Nothing)
        {
    ...
    protected String getTitle()
    {
        ...
        if(titleGuess IsNot Nothing && titleGuess.length() > 0)
        {
            return titleGuess;
        }
        else
        {
            Iterator(of List(Of TextPosition)) textIter = getCharactersByArticle().iterator();
            Single lastFontSize = -1.0f;

            StringBuffer titleText = new StringBuffer();
            while (textIter.hasNext())
            {
                Iterator(Of TextPosition) textByArticle = textIter.next().iterator();
                while (textByArticle.hasNext())
                {
                    TextPosition position = textByArticle.next();

                    Single currentFontSize = position.getFontSize();
    ...
    {
        protected List(Of String) stateList = new ArrayList(Of String)();
        protected Set(Of String) stateSet = new HashSet(Of String)();
         ...
         */
        public String push(String text, List(Of TextPosition) textPositions)
        {
        ...
        private int closeUntil(StringBuilder tagsBuilder, String endTag)
        {
            for (int i = stateList.size(); i-- > 0;)
            {
                ...
                if (endTag IsNot Nothing && tag.equals(endTag))
                {
                    return i;
                }
            }
