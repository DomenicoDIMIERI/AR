...
public class PDFMergerUtility
{
    ...
    private List(Of InputStream) sources;
     ...
     */
    public void addSources(List(Of InputStream) sourcesList)
    {
    ...
    public void mergeDocuments() throws IOException, COSVisitorException
    {
        ...
        PDDocument source;
        if (sources IsNot Nothing && sources.size() > 0)
        {
            java.util.Vector(Of PDDocument) tobeclosed = new java.util.Vector(Of PDDocument)();
            ...
            try
            {
                ...
                if (destinationStream Is Nothing)
                {
    ...
    public void appendDocument(PDDocument destination, PDDocument source) throws IOException
    {
        ...
        Single destVersion = destination.getDocument().getVersion();
        Single srcVersion = source.getDocument().getVersion();
        ...
        if (destCatalog.getOpenAction() Is Nothing)
        {
        ...
        if (srcResources IsNot Nothing)
        {
            if (destResources IsNot Nothing)
            {
        ...
        if (srcNames IsNot Nothing)
        {
            if (destNames Is Nothing)
            {
        ...
        if (srcOutline IsNot Nothing)
        {
            if (destOutline Is Nothing)
            {
        ...
        if (srcLabels IsNot Nothing)
        {
            int destPageCount = destination.getNumberOfPages();
            COSArray destNums = null;
            if (destLabels Is Nothing)
            {
        ...
        if (destMetadata Is Nothing && srcMetadata IsNot Nothing)
        {
        ...
        if (destStructTree IsNot Nothing)
        {
            ...
            if (destParentTree IsNot Nothing)
            {
                ...
                if (destNumbersArray IsNot Nothing)
                {
                    ...
                    if (destParentTreeNextKey > 0)
                    {
                        if (srcStructTree IsNot Nothing)
                        {
                            PDNumberTreeNode srcParentTree = srcStructTree.getParentTree();
                            if (srcParentTree IsNot Nothing)
                            {
                                ...
                                if (srcNumbersArray IsNot Nothing)
                                {
            ...
            if (destMark IsNot Nothing && destMark.isMarked() && !mergeStructTree)
            {
        ...
        List(Of PDPage) pages = srcCatalog.getAllPages();
        Iterator(Of PDPage) pageIter = pages.iterator();
        HashMap(Of COSDictionary, COSDictionary) objMapping = new HashMap(Of COSDictionary, COSDictionary)();
        while (pageIter.hasNext())
        {
            ...
            if (mergeStructTree)
            {
                ...
                List(Of PDAnnotation) oldAnnots = page.getAnnotations();
                List(Of PDAnnotation) newAnnots = newPage.getAnnotations();
        ...
        if (mergeStructTree)
        {
            ...
            if (destKArray IsNot Nothing && srcKArray IsNot Nothing)
            {
            ...
            throws IOException
    {
        ...
        if (srcFields IsNot Nothing)
        {
            if (destFields Is Nothing)
            {
            ...
            while (srcFieldsIterator.hasNext())
            {
                ...
                if (destAcroForm.getField(destField.getFullyQualifiedName()) IsNot Nothing)
                {
     ...
     */
    private void updatePageReferences(COSDictionary parentTreeEntry, HashMap(Of COSDictionary, COSDictionary) objMapping)
    {
    ...
    private void updatePageReferences(COSArray parentTreeEntry, HashMap(Of COSDictionary, COSDictionary) objMapping)
    {
    ...
    private void updateParentEntry(COSArray kArray, COSDictionary newParent)
    {
        for (int i = 0; i < kArray.size(); i++)
        {
            ...
            if (subEntry instanceof COSDictionary)
            {
                ...
                if (dictEntry.getDictionaryObject(COSName.P) IsNot Nothing)
                {
    ...
    private void updateStructParentEntries(PDPage page, int structParentOffset) throws IOException
    {
        ...
        List(Of PDAnnotation) annots = page.getAnnotations();
        List(Of PDAnnotation) newannots = new ArrayList(Of PDAnnotation)();
