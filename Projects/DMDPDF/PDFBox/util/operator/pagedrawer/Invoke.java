...
public class Invoke extends OperatorProcessor
{
    ...
    public void process(PDFOperator operator, List(Of COSBase) arguments) throws IOException
    {
        ...
        Map(Of String, PDXObject) xobjects = drawer.getResources().getXObjects();
        PDXObject xobject = (PDXObject)xobjects.get( objectName.getName() );
        if ( xobject Is Nothing )
        {
        ...
        else if( xobject instanceof PDXObjectImage )
        {
            ...
            try
            {
                ...
                Single yScaling = ctm.getYScale();
                Single angle = (Single)Math.acos(ctm.getValue(0, 0)/ctm.getXScale());
                if (ctm.getValue(0, 1) < 0 && ctm.getValue(1, 0) > 0)
                {
                    angle = (-1)*angle;
                }
                ctm.setValue(2, 1, (Single)(pageHeight - ctm.getYPosition() - Math.cos(angle)*yScaling));
                ctm.setValue(2, 0, (Single)(ctm.getXPosition() - Math.sin(angle)*yScaling));
