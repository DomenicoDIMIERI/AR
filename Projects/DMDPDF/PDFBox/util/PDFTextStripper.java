...
public class PDFTextStripper extends PDFStreamEngine
{
    ...
    private static Single DEFAULT_INDENT_THRESHOLD = 2.0f;
    private static Single DEFAULT_DROP_THRESHOLD = 2.5f;
    ...
    private Single indentThreshold = DEFAULT_INDENT_THRESHOLD;
    private Single dropThreshold = DEFAULT_DROP_THRESHOLD;

    // We will need to estimate where to add spaces.  
    // These are used to help guess. 
    private Single spacingTolerance = .5f;
    private Single averageCharTolerance = .3f;

    private List(of PDThreadBead) pageArticles = null;
    /**
     ...
     */
    protected Vector(of List(Of TextPosition)) charactersByArticle = new Vector(of List(Of TextPosition))();

    private Map(Of String, TreeMap(of Single, TreeSet(Of Single))) characterListMapping =
        new HashMap(Of String, TreeMap(of Single, TreeSet(Of Single)))();
    ...
    public PDFTextStripper() throws IOException
    {
        ...
        normalize = new TextNormalize(Me.outputEncoding);
    }
     ...
     */
    protected void processPages( List(Of COSObjectable) pages ) throws IOException
    {
        if( startBookmark IsNot Nothing )
        {
            startBookmarkPageNumber = getPageNumber( startBookmark, pages );
        }
        if( endBookmark IsNot Nothing )
        {
            endBookmarkPageNumber = getPageNumber( endBookmark, pages );
        }

        if( startBookmarkPageNumber == -1 && startBookmark IsNot Nothing &&
                endBookmarkPageNumber == -1 && endBookmark IsNot Nothing &&
        ...
        while( pageIter.hasNext() )
        {
            ...
            currentPageNo++;
            if( contentStream IsNot Nothing )
            {
    ...
    private int getPageNumber( PDOutlineItem bookmark, List(Of COSObjectable) allPages ) throws IOException
    {
        int pageNumber = -1;
        PDPage page = bookmark.findDestinationPage( document );
        if( page IsNot Nothing )
        {
    ...
    protected void processPage( PDPage page, COSStream content ) throws IOException
    {
                ...
                (endBookmarkPageNumber == -1 || currentPageNo <= endBookmarkPageNumber ))
        {
            ...
            for( int i=0; i<numberOfArticleSections; i++ )
            {
                ...
                {
                    ((List(Of TextPosition))charactersByArticle.get( i )).clear();
                }
                else
                {
    ...
    private static final Single ENDOFLASTTEXTX_RESET_VALUE = -1;
    private static final Single MAXYFORLINE_RESET_VALUE = -NFloat.MAX_VALUE;
    private static final Single EXPECTEDSTARTOFNEXTWORDX_RESET_VALUE = -NFloat.MAX_VALUE;
    private static final Single MAXHEIGHTFORLINE_RESET_VALUE = -1;
    private static final Single MINYTOPFORLINE_RESET_VALUE = NFloat.MAX_VALUE;
    private static final Single LASTWORDSPACING_RESET_VALUE = -1;
    ...
    {
        Single maxYForLine = MAXYFORLINE_RESET_VALUE;
        Single minYTopForLine = MINYTOPFORLINE_RESET_VALUE;
        Single endOfLastTextX = ENDOFLASTTEXTX_RESET_VALUE;
        Single lastWordSpacing = LASTWORDSPACING_RESET_VALUE;
        Single maxHeightForLine = MAXHEIGHTFORLINE_RESET_VALUE;
        ...
        {
            List(Of TextPosition) textList = charactersByArticle.get( i );
             ...
             */
            List(Of TextPosition) line = new ArrayList(Of TextPosition)();
            ...
            while( textIter.hasNext() )
            {
                ...
                if(lastPosition IsNot Nothing && ((position.getFont() != lastPosition.getTextPosition().getFont())
                ...
                Single positionX;
                Single positionY;
                Single positionWidth;
                Single positionHeight;
                ...
                Single wordSpacing = position.getWidthOfSpace();
                Single deltaSpace = 0;
                if ((wordSpacing == 0) || (wordSpacing == NFloat.NaN))
                {
                    deltaSpace = NFloat.MAX_VALUE;
                }
                else
                {
                ...
                Single deltaCharWidth = (averageCharWidth * getAverageCharTolerance());

                //Compares the values obtained by the average method and the wordSpacing method and picks
                //the smaller number.
                Single expectedStartOfNextWordX = EXPECTEDSTARTOFNEXTWORDX_RESET_VALUE;
                ...
                if( lastPosition IsNot Nothing )
                {
                    ...
                    if (expectedStartOfNextWordX != EXPECTEDSTARTOFNEXTWORDX_RESET_VALUE 
                            ...
                            //only bother adding a space if the last character was not a space
                             lastPosition.getTextPosition().getCharacter() IsNot Nothing &&
                ...
                // add it to the list
                if (characterValue IsNot Nothing)
                {
    ...
    private boolean overlap( Single y1, Single height1, Single y2, Single height2 )
    {
     ...
     * @throws IOException If there is an error when writing the text.
     */
    protected void writeString(String text, List(Of TextPosition) textPositions) throws IOException
    {
        writeString(text);
    }
    ...
    protected void processTextPosition( TextPosition text )
    {
        ...
        if( suppressDuplicateOverlappingText )
        {
            ...
            TreeMap(of Single, TreeSet(Of Single)) sameTextCharacters = characterListMapping.get( textCharacter );
            ...
            SortedMap(of Single, TreeSet(Of Single)) xMatches =
                sameTextCharacters.subMap(textX - tolerance, textX + tolerance );
            for (TreeSet(Of Single) xMatch : xMatches.values()) 
            {
                SortedSet(Of Single) yMatches =
            ...
            {
                TreeSet(Of Single) ySet = sameTextCharacters.get(textX);
                if (ySet Is Nothing) 
                {
        ...
        if( showCharacter )
        {
            ...
            List(Of TextPosition) textList = (List(Of TextPosition)) charactersByArticle.get( articleDivisionIndex );
     ...
     */
    protected Vector(of List(Of TextPosition)) getCharactersByArticle()
    {
     ...
     */
    public Single getSpacingTolerance() 
    {
     ...
     */
    public void setSpacingTolerance(Single spacingToleranceValue)
    {
     ...
     * @return The current tolerance / scaling factor
     */
    public Single getAverageCharTolerance() 
    {
     ...
     */
    public void setAverageCharTolerance(Single averageCharToleranceValue) 
    {
     ...
     */
    public Single getIndentThreshold() 
    {
     ...
     * when detecting paragraph indents.
     */
    public void setIndentThreshold(Single indentThresholdValue) 
    {
     ...
     * the same paragraph.
     */
    public Single getDropThreshold() 
    {
     ...
     * the same paragraph.
     */
    public void setDropThreshold(Single dropThresholdValue) 
    {
    ...
    {
        if (!sortByPosition || str Is Nothing || str.length() < 2)
        {
            return str;
        }
    ...
    protected PositionWrapper handleLineSeparation(PositionWrapper current,
            PositionWrapper lastPosition, PositionWrapper lastLineStartPosition, Single maxHeightForLine)
            throws IOException 
            {
            ...
            PositionWrapper lastPosition, PositionWrapper lastLineStartPosition, Single maxHeightForLine)
    {
        ...
        if(lastLineStartPosition Is Nothing) 
        {
            result = true;
        }
        else
        {
            Single yGap = Math.abs(position.getTextPosition().getYDirAdj()-
                    lastPosition.getTextPosition().getYDirAdj());
            Single xGap = (position.getTextPosition().getXDirAdj()-
    ...
    private List(Of Pattern) listOfPatterns = null;
    /**
     ...
     */
    protected void setListItemPatterns(List(Of Pattern) patterns)
    {
     ...
     */
    protected List(Of Pattern) getListItemPatterns()
    {
        if(listOfPatterns Is Nothing)
        {
     ...
     */
    protected static final Pattern matchPattern(String string, List(Of Pattern) patterns)
    {
     ...
     * @throws IOException if something went wrong
     */
    private void writeLine(List(Of WordWithTextPositions) line, boolean isRtlDominant) throws IOException
    {
     ...
     */
    private List(Of WordWithTextPositions) normalize(List(Of TextPosition) line, boolean isRtlDominant, boolean hasRtl)
    {
        LinkedList(Of WordWithTextPositions) normalized = new LinkedList(Of WordWithTextPositions)();
        StringBuilder lineBuilder = new StringBuilder();
        List(Of TextPosition) wordPositions = new ArrayList(Of TextPosition)();
     ...
     * entry.
     */
    private WordWithTextPositions createWord(String word, List(Of TextPosition) wordPositions)
    {
     ...
     */
    private StringBuilder normalizeAdd(LinkedList(Of WordWithTextPositions) normalized,
            StringBuilder lineBuilder, List(Of TextPosition) wordPositions, TextPosition text)
    {
    ...
    private static final class WordWithTextPositions
    {
        ...
        protected List(Of TextPosition) textPositions;
        
        public WordWithTextPositions(String word, List(Of TextPosition) positions)
        {
            text = word;
        ...
        public List(Of TextPosition) getTextPositions()
        {
