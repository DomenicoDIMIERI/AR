...
public class FDFDictionary implements COSObjectable
{
    ...
    {
        PDFileSpecification fs = Me.getFile();
        if( fs IsNot Nothing )
        {
            output.write( "<f href=\"" + fs.getFile() + "\" />\n" );
        }
        COSArray ids = Me.getID();
        ...
        if( fields IsNot Nothing && fields.size() > 0 )
        {
            output.write( "<fields>\n" );
    ...
    public List getFields()
    {
        ...
        if( fieldArray IsNot Nothing )
        {
            List(Of FDFField) fields = new ArrayList(Of FDFField)();
    ...
    public List getEmbeddedFDFs() throws IOException
    {
        ...
        if( embeddedArray IsNot Nothing )
        {
