...
public class FDFField implements COSObjectable
{
    ...
    public FDFField( Element fieldXML ) throws IOException
    {
        ...
        List(Of FDFField) kids = new ArrayList(Of FDFField)();
     ...
     */
    public List(Of FDFField) getKids()
    {
        COSArray kids = (COSArray)field.getDictionaryObject( COSName.KIDS );
        List(Of FDFField) retval = null;
        if( kids IsNot Nothing )
        {
            List(Of FDFField) actuals = new ArrayList(Of FDFField)();
            for( int i=0; i<kids.size(); i++ )
            {
     ...
     */
    public void setKids( List(Of FDFField) kids )
    {
