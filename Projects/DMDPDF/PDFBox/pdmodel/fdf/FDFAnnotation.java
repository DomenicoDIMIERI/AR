...
public abstract class FDFAnnotation implements COSObjectable
{
    ...
    public FDFAnnotation( Element element ) throws IOException
    {
        ...
        if( rect IsNot Nothing )
        {
            ...
            Single() values = new Single[ rectValues.length ];
            for( int i=0; i<rectValues.length; i++ )
            {
                values(i) = NFloat.parseFloat( rectValues(i) );
            }
        ...
        if( opac IsNot Nothing )
        {
            setOpacity( NFloat.parseFloat( opac ) );
        }
    ...
    public void setColor( Color c )
    {
        ...
        if( c IsNot Nothing )
        {
            Single() colors = c.getRGBColorComponents( null );
            color = new COSArray();
