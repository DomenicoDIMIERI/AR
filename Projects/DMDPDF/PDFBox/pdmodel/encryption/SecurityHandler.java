...
public abstract class SecurityHandler
{
    ...
    private Set(Of COSBase) objects = new HashSet(Of COSBase)();

    private Set(Of COSDictionary) potentialSignatures = new HashSet(Of COSDictionary)();
    ...
    protected void proceedDecryption() throws IOException, CryptographyException
    {
        ...
        List(Of COSObject) allObjects = document.getDocument().getObjects();
        Iterator(Of COSObject) objectIter = allObjects.iterator();
    ...
    private void addDictionaryAndSubDictionary(Set(Of COSDictionary) set, COSDictionary dic)
    {
        if (dic IsNot Nothing) // in case dictionary is part of object stream we have null value here
        {
            set.add(dic);
            COSArray kids = (COSArray) dic.getDictionaryObject(COSName.KIDS);
            for (int i = 0; kids IsNot Nothing && i < kids.size(); i++)
            {
    ...
    private void decryptDictionary(COSDictionary dictionary, long objNum, long genNum) throws CryptographyException,
            IOException
    {
        for (Map.Entry(Of COSName, COSBase) entry : dictionary.entrySet())
        {
