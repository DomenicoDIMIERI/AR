...
public class StandardSecurityHandler extends SecurityHandler
{
        ...
        throws CryptographyException, IOException
    {
        ...
        if( documentIDArray IsNot Nothing && documentIDArray.size() >= 1 )
        {
        ...
        if (stdCryptFilterDictionary IsNot Nothing)
        {
            COSName cryptFilterMethod = stdCryptFilterDictionary.getCryptFilterMethod();
            if (cryptFilterMethod IsNot Nothing) 
            {
    ...
    public void prepareDocumentForEncryption(PDDocument doc) throws CryptographyException, IOException
    {
        ...
        if(encryptionDictionary Is Nothing)
        {
        ...
        if( ownerPassword Is Nothing )
        {
            ownerPassword = "";
        }
        if( userPassword Is Nothing )
        {
            userPassword = "";
        }
        ...
        //generate one
        if( idArray Is Nothing || idArray.size() < 2 )
        {
            ...
            try
            {
                ...
                byte[] id = md.digest( Me.toString().getBytes("ISO-8859-1") );
