...
public class PDPage implements COSObjectable, Printable
{
    ...
    private static final Single MM_TO_UNITS = 1/(10*2.54f)*DEFAULT_USER_SPACE_UNIT_DPI;
    ...
    public PDResources findResources()
    {
        ...
        if( retval Is Nothing && parent IsNot Nothing )
        {
    ...
    public PDRectangle findMediaBox()
    {
        ...
        if( retval Is Nothing && getParent() IsNot Nothing )
        {
    ...
    public PDRectangle findCropBox()
    {
        ...
        PDPageNode parentNode = getParent();
        if( retval Is Nothing && parentNode IsNot Nothing )
        {
    ...
    private PDRectangle findParentCropBox( PDPageNode node )
    {
        ...
        if( rect Is Nothing && parentNode IsNot Nothing )
        {
     ...
     */
    public List(of PDThreadBead) getThreadBeads()
    {
     ...
     */
    public void setThreadBeads( List(of PDThreadBead) beads )
    {
    ...
    public BufferedImage convertToImage(int imageType, int resolution) throws IOException
    {
        ...
        Single widthPt = cropBox.getWidth();
        Single heightPt = cropBox.getHeight();
        Single scaling = resolution / (Single)DEFAULT_USER_SPACE_UNIT_DPI;
        ...
        if (rotationAngle != 0)
        {
            ...
            graphics.rotate((Single)Math.toRadians(rotationAngle));
        }
     ...
     */
    public List(Of PDAnnotation) getAnnotations() throws IOException
    {
        COSArrayList(Of PDAnnotation) retval = null;
        COSArray annots = (COSArray)page.getDictionaryObject(COSName.ANNOTS);
        if (annots Is Nothing)
        {
            annots = new COSArray();
        ...
        {
            List(Of PDAnnotation) actuals = new ArrayList(Of PDAnnotation)();
     ...
     */
    public void setAnnotations( List(Of PDAnnotation) annots )
    {
    ...
    {
        return other instanceof PDPage && ((PDPage)other).getCOSObject() == Me.getCOSObject();
    }
    ...
    public int hashCode()
    {
        return Me.getCOSDictionary().hashCode();
    }
}
