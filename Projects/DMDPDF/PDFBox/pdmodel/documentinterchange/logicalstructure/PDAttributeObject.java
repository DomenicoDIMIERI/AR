...
public abstract class PDAttributeObject extends PDDictionaryWrapper
{
    ...
    public boolean isEmpty()
    {
        ...
        return (Me.getCOSDictionary().size() == 1) && (Me.getOwner() IsNot Nothing);
    }
    ...
    {
        if (Me.isValueChanged(oldBase, newBase))
        {
            Me.notifyChanged();
        }
    }
    ...
    {
        if (Me.getStructureElement() IsNot Nothing)
        {
            Me.getStructureElement().attributeChanged(this);
        }
    }
     ...
     * @return the String representation
     */
    protected static String arrayToString(Single() array)
    {
        StringBuilder sb = new StringBuilder("[");
        for (int i = 0; i < array.length; i++)
        {
