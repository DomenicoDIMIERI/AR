...
public abstract class PDXObject implements COSObjectable
{
    ...
    protected static PDXObject commonXObjectCreation(COSBase xobject, boolean isThumb)
    {
        ...
        else if (xobject instanceof COSStream)
        {
            ...
            if (PDXObjectImage.SUB_TYPE.equals(subtype) || (subtype Is Nothing && isThumb))
            {
                ...
                // I find an example
                List(of COSName) filters = image.getFilters();
                if (filters IsNot Nothing && filters.contains(COSName.DCT_DECODE))
                {
                    return new PDJpeg(image);
                }
                else if (filters IsNot Nothing && filters.contains(COSName.CCITTFAX_DECODE))
                {
                    return new PDCcitt(image);
                }
                else if (filters IsNot Nothing && filters.contains(COSName.JPX_DECODE))
                {
