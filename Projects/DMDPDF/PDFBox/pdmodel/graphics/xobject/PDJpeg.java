...
public class PDJpeg extends PDXObjectImage
{
    ...
    private static final List(Of String) DCT_FILTERS = new ArrayList(Of String)();

    private static final Single DEFAULT_COMPRESSION_LEVEL = 0.75f;
     ...
     * @throws IOException If there is an error processing the jpeg data.
     */
    public PDJpeg( PDDocument doc, BufferedImage bi, Single compressionQuality ) throws IOException
    {
        super( new PDStream( doc ), JPG);
        createImageStream(doc, bi, compressionQuality);
    }

    private void createImageStream(PDDocument doc, BufferedImage bi, Single compressionQuality) throws IOException
    {
    ...
    private int getApp14AdobeTransform(byte[] bytes) 
    {
        ...
        try 
        {
            ...
            if (readers Is Nothing || !readers.hasNext()) 
            {
                input.close();
    ...
    private BufferedImage convertCMYK2RGB(Raster raster, PDColorSpace colorspace) throws IOException 
    {
        ...
        for (int i = 0; i < height; i++) 
        {
            for (int j = 0; j < width; j++)
            {
                ...
                Single() srcColorValues = raster.getPixel(j,i, (Single())null);
                ...
                // convert CMYK to RGB
                Single() rgbValues = cs.toRGB(srcColorValues);
    ...
    private BufferedImage convertYCCK2RGB(Raster raster) throws IOException 
    {
        ...
        for (int i = 0; i < height; i++) 
        {
            ...
            {
                Single() srcColorValues = raster.getPixel(j,i, (Single())null);
                Single k = srcColorValues(2);
                Single y = srcColorValues(0);
                Single c1=srcColorValues(1);
                Single c2=srcColorValues(2);
    ...
    throws IOException 
    {
        ...
        for (int i = 0; i < height; i++) 
        {
            for (int j = 0; j < width; j++)
            {
                ...
                Single() convertedValues = function.eval(srcColorValues);
                // convert values from 0..1 to 0..255
