...
public class PDAppearance
{
    ...
    private List(Of COSObjectable) widgets = new ArrayList(Of COSObjectable)();
    ...
    private COSString getDefaultAppearance()
    {
        ...
        if (dap Is Nothing)
        {
            ...
            if( kids IsNot Nothing && kids.size() > 0 )
            {
    ...
    {
        int q = parent.getQ();
        if( parent.getDictionary().getDictionaryObject( COSName.Q ) Is Nothing )
        {
            COSArray kids = (COSArray)parent.getDictionary().getDictionaryObject( COSName.KIDS );
            if( kids IsNot Nothing && kids.size() > 0 )
            {
    ...
    {
        List tokens = null;
        if( appearanceStream IsNot Nothing )
        {
    ...
    public void setAppearanceValue(String apValue) throws IOException
    {
        ...
        while( widgetIter.hasNext() )
        {
            ...
            if( field IsNot Nothing )
            {
                actions = field.getActions();
            }
            if( actions IsNot Nothing &&
                actions.getF() IsNot Nothing &&
            ...
            else
            {
                ...
                if( appearanceStream Is Nothing )
                {
                ...
                {
                    if( tokens IsNot Nothing )
                    {
                        if( daTokens IsNot Nothing )
                        {
                        ...
                        ByteArrayOutputStream output = new ByteArrayOutputStream();
                        ContentStreamWriter writer = new ContentStreamWriter( output );
                        Single fontSize = calculateFontSize( pdFont, appearanceStream.getBoundingBox(), tokens, null );
        ...
        PDFont pdFont, List tokens, PDAppearanceStream appearanceStream ) throws IOException
    {
        ...
        if( defaultAppearance IsNot Nothing )
        {
            ...
            daParser.parse();
            List(Of Object) daTokens = daParser.getTokens();
        ...
        {
            Single fieldWidth = boundingBox.getWidth();
            Single stringWidth = (pdFont.getStringWidth( value )/1000)*fontSize;
            Single adjustAmount = fieldWidth - stringWidth - 4;
    ...
    private PDFont getFontAndUpdateResources( List tokens, PDAppearanceStream appearanceStream ) throws IOException
    {
        ...
        if( formResources IsNot Nothing )
        {
            if( streamResources Is Nothing )
            {
     ...
     */
    private Single getLineWidth( List tokens )
    {
     ...
     */
    private Single calculateFontSize( PDFont pdFont, PDRectangle boundingBox, List tokens, List daTokens )
        throws IOException
    {
        Single fontSize = 0;
        if( daTokens IsNot Nothing )
        ...
        Single widthBasedFontSize = NFloat.MAX_VALUE;
        
        if( parent.doNotScroll() )
        {
            //if we don't scroll then we will shrink the font to fit into the text area.
            Single widthAtFontSize1 = pdFont.getStringWidth( value )/1000.f;
            Single availableWidth = getAvailableWidth(boundingBox, getLineWidth(tokens));
            widthBasedFontSize = availableWidth / widthAtFontSize1;
        }
        else if( fontSize == 0 )
        {
            Single lineWidth = getLineWidth( tokens );
            Single stringWidth = pdFont.getStringWidth( value );
            Single height = 0;
            ...
            Single availHeight = getAvailableHeight( boundingBox, lineWidth );
     ...
     */
    private String getTextPosition( PDRectangle boundingBox, PDFont pdFont, Single fontSize, List tokens )
        throws IOException
    {
        Single lineWidth = getLineWidth( tokens );
        Single pos = 0.0f;
        ...
        else
        {
            if( pdFont instanceof PDSimpleFont )
            {
                ...
                Single bBoxHeight = boundingBox.getHeight();
                Single fontHeight = fd.getFontBoundingBox().getHeight() + 2 * fd.getDescent();
        ...
        Single xInset = 2+ 2*(boundingBox.getWidth() - innerBox.getWidth());
     ...
     */
    private Single getAvailableWidth( PDRectangle boundingBox, Single lineWidth )
    {
     ...
     */
    private Single getAvailableHeight( PDRectangle boundingBox, Single lineWidth )
    {
