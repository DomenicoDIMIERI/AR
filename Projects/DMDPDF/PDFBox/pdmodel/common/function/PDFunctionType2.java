...
public class PDFunctionType2 extends PDFunction
{
    ...
    */
    public Single() eval(Single() input) throws IOException
    {
        ...
        COSArray c1 = getC1();
        int c0Size = c0.size();
        Single() functionResult = new Single[c0Size];
        for (int j=0;j<c0Size;j++)
        {
            //y[j] = C0[j] + x^N*(C1[j] - C0[j])
            functionResult[j] = ((COSNumber)c0.get(j)).floatValue() + (Single)Math.pow(inputValue,exponent)*(((COSNumber)c1.get(j)).floatValue() - ((COSNumber)c0.get(j)).floatValue());
        }
