...
public abstract class PDFunction implements COSObjectable
{
    ...
    {
        if (functionStream IsNot Nothing)
        {
    ...
    public COSArray eval(COSArray input) throws IOException
    {
        ...
        Single() outputValues = eval(input.toFloatArray());
     ...
     * @throws IOException an IOExcpetion is thrown if something went wrong processing the function.  
     */
    public abstract Single() eval(Single() input) throws IOException;
     ...
     */
    protected Single() clipToRange(Single() inputValues) 
    {
        COSArray rangesArray = getRangeValues();
        Single() result = null;
        if (rangesArray IsNot Nothing) 
        {
            Single() rangeValues = rangesArray.toFloatArray();
            int numberOfRanges = rangeValues.length/2;
            result = new Single[numberOfRanges];
     ...
     */
    protected Single clipToRange(Single x, Single rangeMin, Single rangeMax) 
    {
     ...
     */
    protected Single interpolate(Single x, Single xRangeMin, Single xRangeMax, Single yRangeMin, Single yRangeMax) 
    {
