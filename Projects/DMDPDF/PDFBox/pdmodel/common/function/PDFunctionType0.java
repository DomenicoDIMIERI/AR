...
public class PDFunctionType0 extends PDFunction
{
    ...
    public PDRange getEncodeForParameter( int paramNum )
    {
        ...
        if( encodeValues IsNot Nothing && encodeValues.size() >= paramNum*2+1 )
        {
    ...
    public PDRange getDecodeForParameter( int paramNum )
    {
        ...
        if( decodeValues IsNot Nothing && decodeValues.size() >= paramNum*2+1 )
        {
    ...
    */
    public Single() eval(Single() input) throws IOException
    {
        ...
        Single() sizeValues = getSize().toFloatArray();
        int bitsPerSample = getBitsPerSample();
        ...
        Single() outputValuesPrevious = null;
        Single() outputValuesNext = null;
        outputValuesPrevious = getSample(intInputValuesPrevious);
        outputValuesNext = getSample(intInputValuesNext);
        Single() outputValues = new Single[numberOfOutputValues];
        for (int i=0;i<numberOfOutputValues;i++) 
        {
            ...
            outputValues(i) = interpolate(outputValues(i), 0, (Single)Math.pow(2, bitsPerSample), decode.getMin(), decode.getMax());
     ...
     */
    private Single() getSample(int indexValue)
    {
        int[] sampleValues = getSamples()[indexValue];
        int numberOfValues = sampleValues.length;
        Single() result = new Single[numberOfValues];
