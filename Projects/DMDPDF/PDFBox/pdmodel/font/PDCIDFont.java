...
public abstract class PDCIDFont extends PDSimpleFont
{
    ...
    private Map<Integer,NFloat> widthCache = null;
    ...
    public Single getFontWidth( byte[] c, int offset, int length ) throws IOException
    {
        ...
        Single retval = getDefaultWidth();
        int code = getCodeFromArray( c, offset, length );

        NFloat widthFloat = widthCache.get( code );
        if( widthFloat IsNot Nothing )
        {
    ...
    {
        if (widthCache Is Nothing) 
        {
            widthCache = new HashMap<Integer,NFloat>();
            ...
            if( widths IsNot Nothing )
            {
                ...
                while (counter < size) 
                {
                    ...
                    else
                    {
                        ...
                        Single width = rangeWidth.floatValue();
    ...
    public Single getFontHeight( byte[] c, int offset, int length ) throws IOException
    {
        ...
        Single xHeight = desc.getXHeight();
        Single capHeight = desc.getCapHeight();
    ...
    {
        Single totalWidths = 0.0f;
        Single characterCount = 0.0f;
        Single defaultWidth = getDefaultWidth();
        COSArray widths = (COSArray)font.getDictionaryObject( COSName.W );
        ...
        Single average = totalWidths / characterCount;
        if( average <= 0 )
        {
    ...
    private String getCIDSystemInfo()
    {
        ...
        if (cidsysteminfo IsNot Nothing) 
        {
