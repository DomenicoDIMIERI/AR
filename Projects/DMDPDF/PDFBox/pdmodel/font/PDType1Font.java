...
public class PDType1Font extends PDSimpleFont
{
        ...
        if( awtFont Is Nothing )
        {
            ...
            else
            {
                ...
                if (fd IsNot Nothing && fd instanceof PDFontDescriptorDictionary)
                {
                    ...
                    if( fdDictionary.getFontFile() IsNot Nothing )
                    {
                        try 
                        {
    ...
    private void getEncodingFromFont(boolean extractEncoding)
    {
        ...
        if( fontDescriptor IsNot Nothing && fontDescriptor instanceof PDFontDescriptorDictionary)
        {
            PDStream fontFile = ((PDFontDescriptorDictionary)fontDescriptor).getFontFile();
            if( fontFile IsNot Nothing )
            {
                ...
                try 
                {
                    ...
                    while( (line = in.readLine()) IsNot Nothing)
                    {
                        if (extractEncoding) 
                        {
                            ...
                            if (line.startsWith("/Encoding")) 
                            {
                                ...
                                // assign another one
                                else if (getFontEncoding() Is Nothing)
                                {
                                    StringTokenizer st = new StringTokenizer(line);
                                    // ignore the first token
                                    st.nextElement();
                        ...
                        if (line.startsWith("/FontMatrix"))
                        {
                            ...
                            if (line.indexOf("[") > -1)
                            {
                                ...
                                if (st.countTokens() >= 6)
                                {
                                    try 
                                    {
                                        for (int i=0;i<6;i++)
                                        {
                                            COSFloat floatValue = new COSFloat(NFloat.parseFloat(st.nextToken()));
                            ...
                            else
                            {
                                ...
                                while((line = in.readLine()) IsNot Nothing)
                                {
                                    ...
                                    try
                                    {
                                        COSFloat floatValue = new COSFloat(NFloat.parseFloat(line));
                                        array.add(floatValue);
                                    }
                                    catch (NumberFormatException exception)
                                    {
    ...
    {
        if (type1CFont IsNot Nothing && getFontEncoding() Is Nothing)
        {
