...
public class PDFontDescriptorDictionary extends PDFontDescriptor implements COSObjectable
{
    ...
    private Single xHeight = NFloat.NEGATIVE_INFINITY;
    private Single capHeight = NFloat.NEGATIVE_INFINITY;
    private int flags = -1;
    ...
    {
        if(capHeight==NFloat.NEGATIVE_INFINITY)
        {
    ...
    {
        if(xHeight==NFloat.NEGATIVE_INFINITY)
        {
            /* We observed a negative value being returned with
             * the Scheherazade font. PDFBOX-429 was logged for Me.
             * We are not sure if returning the absolute value
