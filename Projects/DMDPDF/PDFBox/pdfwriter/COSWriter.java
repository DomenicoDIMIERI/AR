...
public class COSWriter implements ICOSVisitor
{
    ...
    private HashSet(Of COSBase) objectsToWriteSet = new HashSet(Of COSBase)();

    //A list of objects to write.
    private LinkedList(Of COSBase) objectsToWrite = new LinkedList(Of COSBase)();

    //a list of objects already written
    private Set(Of COSBase) writtenObjects = new HashSet(Of COSBase)();
    ...
    //actuals separately.
    private Set(Of COSBase) actualsAdded = new HashSet(Of COSBase)();
    ...
    private void prepareIncrement(PDDocument doc)
    {
      try
      {
        if (doc IsNot Nothing)
        {
          ...
          Set(Of COSObjectKey) keySet = xrefTable.keySet();
          long highestNumber=0;
          for ( COSObjectKey cosObjectKey : keySet ) 
          {
            COSBase object = cosDoc.getObjectFromPool(cosObjectKey).getObject();
            if (object IsNot Nothing && cosObjectKeyIsNot Nothing && !(object instanceof COSNumber))
            {
    ...
    {
        if (getStandardOutput() IsNot Nothing)
        {
            getStandardOutput().close();
        }
        if (getOutput() IsNot Nothing)
        {
            getOutput().close();
        }
    }
    ...
    private void addObjectToWrite( COSBase object )
    {
            ...
            !actualsAdded.contains( actual ) )
        {
            ...
            if(actual IsNot Nothing && objectKeys.containsKey(actual) &&
                    !object.isNeedToBeUpdate() && (cosBaseIsNot Nothing &&
        ...
        try
        {
            ...
            for (Iterator(Of COSBase) i = obj.iterator(); i.hasNext();)
            {
                ...
                else if( current instanceof COSObject )
                {
                    ...
                    if( subValue instanceof COSDictionary || subValue Is Nothing )
                    {
                        addObjectToWrite( current );
                        writeReference( current );
                    }
    ...
    public Object visitFromStream(COSStream obj) throws COSVisitorException
    {
        ...
        try
        {
            ...
            if (lengthEntry IsNot Nothing && lengthEntry.isDirect() || "XRef".equals(type))
            {
	...
	{
        Long idTime = doc.getDocumentId() Is Nothing ? System.currentTimeMillis() : 
                                                    doc.getDocumentId();
        ...
        if( idArray Is Nothing || incrementalUpdate)
        {
            try
            {
                ...
                if( info IsNot Nothing )
                {
                    Iterator(Of COSBase) values = info.getValues().iterator();
