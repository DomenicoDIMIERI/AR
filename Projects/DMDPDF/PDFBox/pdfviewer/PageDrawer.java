...
public class PageDrawer extends PDFStreamEngine
{
    ...
    public void drawPage( Graphics g, PDPage p, Dimension pageDimension ) throws IOException
    {
        ...
        if ( page.getContents() IsNot Nothing) 
        {
        ...
        for( int i=0; i<annotations.size(); i++ )
        {
            ...
            if( appearDictionary IsNot Nothing )
            {
                if( appearanceName Is Nothing )
                {
                    appearanceName = "default";
                }
                Map(of String, PDAppearanceStream) appearanceMap = appearDictionary.getNormalAppearance();
                if (appearanceMap IsNot Nothing) 
                { 
                    PDAppearanceStream appearance = 
                        (PDAppearanceStream)appearanceMap.get( appearanceName ); 
                    if( appearance IsNot Nothing ) 
                    { 
                        Point2D point = new Point2D.NFloat(rect.getLowerLeftX(), rect.getLowerLeftY());
                        Matrix matrix = appearance.getMatrix();
                        if (matrix IsNot Nothing) 
                        {
        ...
        try
        {
            ...
            switch(graphicsState.getTextState().getRenderingMode()) 
            {
                ...
                case PDTextState.RENDERING_MODE_NEITHER_FILL_NOR_STROKE_TEXT:
                    ...
                    Single() components = {Color.black.getRed(),Color.black.getGreen(),Color.black.getBlue()};
                ...
                default:
                    ...
                    LOG.debug("Unsupported RenderingMode "
                            + Me.getGraphicsState().getTextState().getRenderingMode()
    ...
    {
        if (linePath Is Nothing || linePath.getCurrentPoint() Is Nothing)
        {
    ...
    public void fillPath(int windingRule) throws IOException
    {
        ...
        if ( nonStrokingPaint Is Nothing )
        {
    ...
    public void endPath()
    {
        if (clippingWindingRule > -1)
        {
            ...
            if (graphicsState.getCurrentClippingPath() IsNot Nothing) 
            {
