...
public class OverlayPDF 
{
    ...
    {
        Map(Of NInteger, String) specificPageOverlayFile = new HashMap(Of NInteger, String)();
        ...
        for (int i = 0; i < args.length; i++) 
        {
            ...
            else if (defaultOverlayFile Is Nothing) 
            {
        ...
        if ((inputFile Is Nothing) || (outputFile Is Nothing)) 
        {
            usage();
        }
     ...
     */
    public void overlay(Map(Of NInteger, String) specificPageOverlayFile) throws IOException, COSVisitorException 
    {
        ...
        try 
        {
            ...
            if (defaultOverlayFile IsNot Nothing) 
            {
                if (useNonSeqParser)
                {
            ...
            if (firstPageOverlayFile IsNot Nothing) 
            {
                if (useNonSeqParser)
                {
            ...
            if (lastPageOverlayFile IsNot Nothing) 
            {
                if (useNonSeqParser)
                {
            ...
            if (oddPageOverlayFile IsNot Nothing) 
            {
                if (useNonSeqParser)
                {
            ...
            if (evenPageOverlayFile IsNot Nothing) 
            {
                if (useNonSeqParser)
                {
            ...
            for (Map.Entry(Of NInteger, String) e : specificPageOverlayFile.entrySet()) 
            {
                PDDocument doc = null;
                if (useNonSeqParser)
                {
        ...
        {
            if (sourcePDFDocument IsNot Nothing) 
            {
                sourcePDFDocument.close();
            }
            if (defaultOverlay IsNot Nothing) 
            {
                defaultOverlay.close();
            }
            if (firstPageOverlay IsNot Nothing) 
            {
                firstPageOverlay.close();
            }
            if (lastPageOverlay IsNot Nothing) 
            {
                lastPageOverlay.close();
            }
            if (oddPageOverlay IsNot Nothing) 
            {
                oddPageOverlay.close();
            }
            if (evenPageOverlay IsNot Nothing) 
            {
    ...
    private void overlayPage(COSArray array, PDPage page, int pageNumber, int numberOfPages) throws IOException 
    {
        ...
        else if ((pageNumber == 1) && (firstPageOverlayPage IsNot Nothing)) 
        {
            layoutPage = firstPageOverlayPage;
        } 
        else if ((pageNumber == numberOfPages) && (lastPageOverlayPage IsNot Nothing)) 
        {
            layoutPage = lastPageOverlayPage;
        } 
        else if ((pageNumber % 2 == 1) && (oddPageOverlayPage IsNot Nothing)) 
        {
            layoutPage = oddPageOverlayPage;
        } 
        else if ((pageNumber % 2 == 0) && (evenPageOverlayPage IsNot Nothing)) 
        {
            layoutPage = evenPageOverlayPage;
        } 
        else if (defaultOverlayPage IsNot Nothing) 
        {
            layoutPage = defaultOverlayPage;
        }
        if (layoutPage IsNot Nothing) 
        {
            PDResources resources = page.findResources();
            if (resources Is Nothing) 
            {
    ...
    private COSStream createOverlayStream(PDPage page, LayoutPage layoutPage, String xObjectId) throws IOException 
    {
        ...
        Single scale = 1;
        Single hShift = (pageMediaBox.getWidth() - layoutPage.overlayMediaBox.getWidth()) / 2.0f;
        Single vShift = (pageMediaBox.getHeight() - layoutPage.overlayMediaBox.getHeight()) / 2.0f;
