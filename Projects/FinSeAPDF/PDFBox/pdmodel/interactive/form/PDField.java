...
public abstract class PDField implements COSObjectable
{
    ...
    public void importFDF(FDFField fdfField) throws IOException
    {
        ...
        List(Of FDFField) fdfKids = fdfField.getKids();
        List(Of COSObjectable) pdKids = getKids();
        for (int i = 0; fdfKids IsNot Nothing && i < fdfKids.size(); i++)
        {
            ...
            for (int j = 0; j < pdKids.size(); j++)
            {
                ...
                if (pdChildObj instanceof PDField)
                {
                    ...
                    if (fdfName IsNot Nothing && fdfName.equals(pdChild.getPartialName()))
                    {
    ...
    public PDAnnotationWidget getWidget() throws IOException
    {
        ...
        List(Of COSObjectable) kids = getKids();
        if (kids Is Nothing)
        {
    ...
    public PDField findKid(String[] name, int nameIndex) throws IOException
    {
        ...
        {
            for (int i = 0; retval Is Nothing && i < kids.size(); i++)
            {
     ...
     */
    public List(Of COSObjectable) getKids() throws IOException
    {
        List(Of COSObjectable) retval = null;
        COSArray kids = (COSArray) getDictionary().getDictionaryObject(COSName.KIDS);
        if (kids IsNot Nothing)
        {
            List(Of COSObjectable) kidsList = new ArrayList(Of COSObjectable)();
            for (int i = 0; i < kids.size(); i++)
            {
                COSDictionary kidDictionary = (COSDictionary) kids.getObject(i);
                if (kidDictionary Is Nothing)
                {
                    continue;
                }
                COSDictionary parent = (COSDictionary) kidDictionary.getDictionaryObject(COSName.PARENT, COSName.P);
                if (kidDictionary.getDictionaryObject(COSName.FT) IsNot Nothing
                        || (parent IsNot Nothing && parent.getDictionaryObject(COSName.FT) IsNot Nothing))
                {
     ...
     */
    public void setKids(List(Of COSObjectable) kids)
    {
