...
public class RadialShadingContext implements PaintContext 
{
            ...
            AffineTransform xform, Matrix ctm, int pageHeight) 
    {
        ...
        // domain values
        if (shadingType3.getDomain() IsNot Nothing)
        {
        ...
        if (shadingType3.getExtend() IsNot Nothing)
        {
    ...
    public Raster getRaster(int x, int y, int w, int h) 
    {
        ...
        for (int j = 0; j < h; j++) 
        {
            ...
            {
                int index = (j * w + i) * 3;
                Single() inputValues = calculateInputValues(x + i, y + j);
    ...
    private Single() calculateInputValues(int x, int y) 
    {
        ...
        values(0) = (Single)((-1)*p + root);
        values(1) = (Single)((-1)*p - root);
        return values;
    }
}
