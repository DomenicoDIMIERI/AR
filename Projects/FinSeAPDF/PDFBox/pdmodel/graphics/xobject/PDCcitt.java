...
public class PDCcitt extends PDXObjectImage
{
    ...
    private static final List(Of String) FAX_FILTERS = new ArrayList(Of String)();
    ...
    public BufferedImage getRGBImage() throws IOException
    {
        ...
        if (decode IsNot Nothing && decode.getInt(0) == 1)
        {
        ...
        if (colorspace instanceof PDIndexed)
        {
            ...
            if (maskArray IsNot Nothing && maskArray instanceof COSArray)
            {
        ...
        private void buildHeader(COSDictionary options)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < parmsArray.size() && decodeParms Is Nothing; i++)
                    {
                        COSDictionary dic = (COSDictionary) parmsArray.getObject(i);
                        if (dic IsNot Nothing
                                && (dic.getDictionaryObject(COSName.COLUMNS) IsNot Nothing || dic
                                        .getDictionaryObject(COSName.ROWS) IsNot Nothing))
                        {
                            decodeParms = dic;
                        }
                    }
