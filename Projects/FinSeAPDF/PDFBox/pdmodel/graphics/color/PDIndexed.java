...
public class PDIndexed extends PDColorSpace
{
    ...
    public byte[] getLookupData() throws IOException
    {
        if (lookupData Is Nothing)
        {
            ...
            else if (lookupTable Is Nothing)
            {
     ...
     */
    public Single() calculateColorValues(int index) throws IOException
    {
        // TODO bpc != 8 ??
        calculateIndexedColorValues(getBaseColorModel(INDEXED_BPC), 8);
        Single() colorValues = null;
        if (index < maxIndex)
        {
            int bufferIndex = index * indexNumOfComponents;
            colorValues = new Single[indexNumOfComponents];
            for (int i = 0; i < indexNumOfComponents; i++)
            {
                colorValues(i) = (Single) indexedColorValues[bufferIndex + i];
            }
        }
        return colorValues;
    }
    ...
    {
        if (indexedColorValues Is Nothing)
        {
