...
public abstract class PDStructureNode implements COSObjectable
{
    ...
    public static PDStructureNode create(COSDictionary node)
    {
        ...
        if ((type Is Nothing) || "StructElem".equals(type))
        {
     ...
     */
    public List(Of Object) getKids()
    {
        List(Of Object) kidObjects = new ArrayList(Of Object)();
        ...
        if (k instanceof COSArray)
        {
            ...
            while (kids.hasNext())
            {
                ...
                Object kidObject = Me.createObject(kid);
                if (kidObject IsNot Nothing)
                {
        ...
        {
            Object kidObject = Me.createObject(k);
            if (kidObject IsNot Nothing)
            {
                kidObjects.add(kidObject);
            }
     ...
     * @param kids the kids
     */
    public void setKids(List(Of Object) kids)
    {
        Me.getCOSDictionary().setItem(COSName.K,
    ...
    {
        if ((newKid Is Nothing) || (refKid Is Nothing))
        {
            return;
        }
        COSBase k = Me.getCOSDictionary().getDictionaryObject(COSName.K);
    ...
    {
        boolean removed = Me.removeObjectableKid(structureElement);
        if (removed)
        {
    ...
    protected boolean removeKid(COSBase object)
    {
        ...
        else
        {
            ...
            if (onlyKid)
            {
                Me.getCOSDictionary().setItem(COSName.K, null);
                return true;
            }
            return false;
        }
    ...
    protected Object createObject(COSBase kid)
    {
        ...
        if (kidDic IsNot Nothing)
        {
            String type = kidDic.getNameAsString(COSName.TYPE);
            if ((type Is Nothing) || PDStructureElement.TYPE.equals(type))
            {
