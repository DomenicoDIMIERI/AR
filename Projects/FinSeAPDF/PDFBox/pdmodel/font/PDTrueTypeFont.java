...
public class PDTrueTypeFont extends PDSimpleFont
{
    ...
    private void loadDescriptorDictionary(PDFontDescriptorDictionary fd, InputStream ttfData) throws IOException
    {
        ...
        try
        {
            ...
            Single scaling = 1000f/header.getUnitsPerEm();
            ...
            Map(Of NInteger, String) codeToName = Me.getFontEncoding().getCodeToNameMap();
            ...
            List(Of Single) widths = new ArrayList(Of Single)(nWidths);
            ...
            {
                widths.add((Single)defaultWidth);
            }
            ...
            //
            for (Entry(Of NInteger, String) e : codeToName.entrySet()) 
            {
                ...
                if (gid != 0) 
                {
                    if (isMonospaced)
                    {
                        widths.set(e.getKey().intValue() - firstChar, (Single)defaultWidth);
                    }
                    else
                    {
                        widths.set(e.getKey().intValue() - firstChar, (Single)Math.round(widthValues[gid] * scaling));
                    }
                }
            }
            setWidths( widths );
    ...
    private InputStream getExternalTTFData() throws IOException
    {
        ...
        if( baseFont IsNot Nothing && externalFonts.containsKey(baseFont) )
        {
            ttfResource = externalFonts.getProperty( baseFont );
        }
        return (ttfResource IsNot Nothing ? ResourceLoader.loadResource(ttfResource) : null);
    }
