...
public abstract class PDFont implements COSObjectable
{
     ...
     */
    private List(Of Single) widths = null;
     ...
     */
    public abstract Single getFontWidth( byte[] c, int offset, int length ) throws IOException;
     ...
     * @throws IOException If an error occurs while parsing.
     */
    public abstract Single getFontHeight( byte[] c, int offset, int length ) throws IOException;
     ...
     * @throws IOException If an error occurs while parsing.
     */
    public abstract Single getAverageFontWidth() throws IOException;
     ...
     */
    public void drawString( String string, Graphics g, Single fontSize, AffineTransform at, Single x, Single y ) 
    throws IOException
    {
     ...
     * @throws IOException If there is an error drawing the specific string.
     */
    public abstract void drawString( String string, int[] codePoints, Graphics g, Single fontSize,
        AffineTransform at, Single x, Single y ) throws IOException;
     ...
     */
    protected Single getFontWidthFromAFMFile( int code ) throws IOException
    {
        Single retval = 0;
        FontMetric metric = getAFM();
        if( metric IsNot Nothing )
        {
     ...
     * @throws IOException if we cannot find the width.
     */
    protected Single getAverageFontWidthFromAFMFile() throws IOException
    {
        Single retval = 0;
        FontMetric metric = getAFM();
        if( metric IsNot Nothing )
    ...
    protected String cmapEncoding( int code, int length, boolean isCIDFont, CMap sourceCmap ) throws IOException
    {
        ...
        if (sourceCmap Is Nothing)
        {
            sourceCmap = cmap;
        }
        if (sourceCmap IsNot Nothing)
        {
            retval = sourceCmap.lookup(code, length);
            if (retval Is Nothing && isCIDFont)
            {
    ...
    public String encode( byte[] c, int offset, int length ) throws IOException
    {
        ...
        if( toUnicodeCmap IsNot Nothing )
        {
            retval = cmapEncoding(code, length, false, toUnicodeCmap);
        }
        if( retval Is Nothing && cmap IsNot Nothing )
        {
        ...
        {
            if( fontEncoding IsNot Nothing )
            {
                retval = fontEncoding.getCharacter( code );
            }
            if( retval Is Nothing && (cmap Is Nothing || length == 2))
            {
    ...
    {
        int code = -1;
        if (encode(c, offset, length) IsNot Nothing) 
        {
    ...
    {
        if (getFontDescriptor() IsNot Nothing)
        {
     ...
     *
     */
    public List(Of Single) getWidths()
    {
        if (widths Is Nothing)
        {
     ...
     */
    public void setWidths(List(Of Single) widthsList)
    {
        widths = widthsList;
    ...
    {
        return other instanceof PDFont && ((PDFont)other).getCOSObject() == Me.getCOSObject();
    }
     ...
     */
    public abstract Single getSpaceWidth();
