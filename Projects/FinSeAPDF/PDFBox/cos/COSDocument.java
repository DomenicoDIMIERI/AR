...
public class COSDocument extends COSBase
{
        ...
        for( COSObject object : objectPool.values() )
        {
            ...
            if( realObject instanceof COSDictionary )
            {
                try
                {
                    ...
                    if( objectType IsNot Nothing && objectType.equals( type ) )
                    {
                        return object;
                    }
                }
     ...
     * @throws IOException If there is an error getting the object
     */
    public List(Of COSObject) getObjectsByType( String type ) throws IOException
    {
     ...
     * @throws IOException If there is an error getting the object
     */
    public List(Of COSObject) getObjectsByType( COSName type ) throws IOException
    {
        List(Of COSObject) retval = new ArrayList(Of COSObject)();
        for( COSObject object : objectPool.values() )
        {
            COSBase realObject = object.getObject();
            if( realObject instanceof COSDictionary )
     ...
     */
    public void setVersion( Single versionValue )
    {
     ...
     */
    public List(Of COSDictionary) getSignatureDictionaries() throws IOException
    {
        List(Of COSDictionary) signatureFields = getSignatureFields(false);
        List(Of COSDictionary) signatures = new LinkedList(Of COSDictionary)();
        for ( COSDictionary dict : signatureFields )
        {
            COSBase dictionaryObject = dict.getDictionaryObject(COSName.V);
            if (dictionaryObject IsNot Nothing)
            {
     ...
     * @throws IOException if no document catalog can be found
     */
    public List(Of COSDictionary) getSignatureFields(boolean onlyEmptyFields) throws IOException
    {
        COSObject documentCatalog = getCatalog();
        if (documentCatalog IsNot Nothing)
        {
            ...
            if (acroForm IsNot Nothing)
            {
                ...
                if (fields IsNot Nothing)
                {
                    ...
                    for ( Object object : fields )
                    {
                        ...
                        {
                            COSBase dictionaryObject = dict.getDictionaryObject(COSName.V);
                            if (dictionaryObject Is Nothing || (dictionaryObject IsNot Nothing && !onlyEmptyFields))
                            {
     ...
     */
    public List(Of COSObject) getObjects()
    {
    ...
    public void dereferenceObjectStreams() throws IOException
    {
        for( COSObject objStream : getObjectsByType( COSName.OBJ_STM ) )
        {
            ...
            for( COSObject next : parser.getObjects() )
            {
                ...
                if ( objectPool.get(key) Is Nothing || objectPool.get(key).getObject() Is Nothing ||
