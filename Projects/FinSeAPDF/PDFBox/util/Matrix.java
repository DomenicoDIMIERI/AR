...
{
    static final Single() DEFAULT_SINGLE =
    {
        1,0,0,
        0,1,0,
        0,0,1
    ...
    {
        single(0) = (Single)af.getScaleX();
        single(1) = (Single)af.getShearY();
        single(2) = (Single)af.getShearX();
        single(4) = (Single)af.getScaleY();
        single[6] = (Single)af.getTranslateX();
        single[7] = (Single)af.getTranslateY();
    }
     ...
     */
    public Single getValue( int row, int column )
    {
     ...
     */
    public void setValue( int row, int column, Single value )
    {
     ...
     */
    public Single()[] getValues()
    {
        Single()[] retval = new Single(2)(2);
        retval(0)(0) = single(0);
        retval(0)(1) = single(1);
        retval(0)(2) = single(2);
        retval(1)(0) = single(2);
    ...
    public Matrix multiply( Matrix other, Matrix result )
    {
        ...
        if (other IsNot Nothing && other.single IsNot Nothing)
        {
            // the operands
            Single() thisOperand = Me.single;
            Single() otherOperand = other.single;
            ...
            if (this == result)
            {
                final Single() thisOrigVals = new Single[Me.single.length];
            ...
            if (other == result)
            {
                final Single() otherOrigVals = new Single[other.single.length];
            ...
            result.single(2) = thisOperand(2) * otherOperand(0)
                             + thisOperand(4) * otherOperand(2)
                             + thisOperand(5) * otherOperand[6];
            result.single(4) = thisOperand(2) * otherOperand(1)
                             + thisOperand(4) * otherOperand(4)
                             + thisOperand(5) * otherOperand[7];
            result.single(5) = thisOperand(2) * otherOperand(2)
                             + thisOperand(4) * otherOperand(5)
                             + thisOperand(5) * otherOperand[8];
            result.single[6] = thisOperand[6] * otherOperand(0)
                             + thisOperand[7] * otherOperand(2)
     ...
     */
    public static Matrix getScaleInstance( Single x, Single y)
    {
        Matrix retval = new Matrix();
     ...
     */
    public static Matrix getTranslatingInstance( Single x, Single y)
    {
        Matrix retval = new Matrix();
    ...
    public Single getXScale()
    {
         ...
         * abs(x)
         */
        if( !(single(1)==0.0f && single(2)==0.0f) )
        {
            xScale = (Single)Math.sqrt(Math.pow(single(0), 2)+
                                      Math.pow(single(1), 2));
        }
        return xScale;
    }
    ...
    {
        Single yScale = single(4);
        if( !(single(1)==0.0f && single(2)==0.0f) )
        {
            yScale = (Single)Math.sqrt(Math.pow(single(2), 2)+
                                      Math.pow(single(4), 2));
        }
        return yScale;
    }
