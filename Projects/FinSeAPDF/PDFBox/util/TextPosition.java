...
public class TextPosition
{
    ...
    private Single maxTextHeight; // maximum height of text, in display units
    private int rot; // 0, 90, 180, 270 degrees of page rotation
    private Single x = NFloat.NEGATIVE_INFINITY;
    private Single y = NFloat.NEGATIVE_INFINITY;
    private Single pageHeight;
    private Single pageWidth;
    private Single() widths;
    private Single widthOfSpace; // width of a space, in display units
    private String str;
    ...
    private Single wordSpacing;  // word spacing value, in display units
    ...
    public TextPosition(
            ...
            Single maxFontH,
            Single() individualWidths,
            Single spaceWidth,
            String string,
            PDFont currentFont,
            Single fontSizeValue,
            int fontSizeInPt,
            Single ws
    )
    {
    ...
    public TextPosition(
            int pageRotation,
            Single pageWidthValue,
            Single pageHeightValue,
            Matrix textPositionSt,
            Matrix textPositionEnd,
            Single maxFontH,
            Single individualWidth,
            Single spaceWidth,
            String string,
            PDFont currentFont,
            Single fontSizeValue,
     ...
     */
    public Single getDir()
    {
        Single a = textPos.getValue(0,0);
        Single b = textPos.getValue(0,1);
        Single c = textPos.getValue(1,0);
        Single d = textPos.getValue(1,1);
     ...
     * @return X coordinate
     */
    private Single getXRot(Single rotation)
    {
        if (rotation == 0)
        {
     ...
     */
    private Single getYLowerLeftRot(Single rotation)
    {
        if (rotation == 0)
        {
            return textPos.getValue(2,1);
     ...
     */
    private Single getWidthRot(Single rotation)
    {
     ...
     */
    public Single() getIndividualWidths()
    {
        return widths;
    }
    ...
    public void mergeDiacritic(TextPosition diacritic, TextNormalize normalize)
    {
        ...
        Single diacXStart = diacritic.getXDirAdj();
        Single diacXEnd = diacXStart + diacritic.widths(0);

        Single currCharXStart = getXDirAdj();
        ...
        {
            Single currCharXEnd = currCharXStart + widths(i);
            ...
            if(diacXStart < currCharXStart && diacXEnd <= currCharXEnd)
            {
                ...
                {
                    Single distanceOverlapping1 = diacXEnd - currCharXStart;
                    Single percentage1 = distanceOverlapping1/widths(i);

                    Single distanceOverlapping2 = currCharXStart - diacXStart;
                    Single percentage2 = distanceOverlapping2/widths[i-1];
    ...
    private void insertDiacritic(int i, TextPosition diacritic, TextNormalize normalize)
    {
        ...
        Single() widths2 = new Single[widths.length+1];
    ...
    {
        final String cText = Me.getCharacter(); 
        if (cText.length() != 1)
        {
            return false; 
        }
