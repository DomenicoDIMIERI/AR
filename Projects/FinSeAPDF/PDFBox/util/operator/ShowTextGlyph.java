...
public class ShowTextGlyph extends OperatorProcessor
{
    ...
    public void process(PDFOperator operator, List(Of COSBase) arguments) throws IOException
    {
        ...
        Single fontsize = context.getGraphicsState().getTextState().getFontSize();
        Single horizontalScaling = context.getGraphicsState().getTextState().getHorizontalScalingPercent()/100;
        for( int i=0; i<arraySize; i++ )
        {
            COSBase next = array.get( i );
            if( next instanceof COSNumber )
            {
                Single adjustment = ((COSNumber)next).floatValue();
