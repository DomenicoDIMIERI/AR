...
public class PDFParser extends BaseParser
{
    ...
    public void parse() throws IOException
    {
        try
        {
            if ( raf Is Nothing )
            {
                if( tempDirectory IsNot Nothing )
                {
    ...
    private void parseHeader() throws IOException
    {
        ...
        try
        {
            if (header.startsWith( PDF_HEADER ))
            {
                Single pdfVersion = NFloat. parseFloat(
        ...
        else
        {
            ...
            if( endObjectKey.equals( "stream" ) )
            {
                ...
                if( pb instanceof COSDictionary )
                {
                    ...
                    if( objectType IsNot Nothing && objectType.equals( COSName.XREF ) )
                    {
                        // XRef stream
    ...
    */
    ...
    private void readVersionInTrailer(COSDictionary parsedTrailer)
    {
        ...
        if (root IsNot Nothing)
        {
            ...
            if (version IsNot Nothing)
            {
                Single trailerVersion = NFloat.valueOf(version.getName());
            ...
            if (conflicts.hasNext())
            {
                ...
                do
                {
                    ...
                    if (tolerantConflicResolver(values, offset, 4))
                    {
                        ...
                        if (pdfObject.getObjectNumber() IsNot Nothing 
