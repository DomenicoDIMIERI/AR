            ...
            For i As Integer = 0 To altriPrestiti.Count() - 1
                ...
                If (Not trovato) Then
                    ...
                    If (est.Scadenza.HasValue = False AndAlso est.DataInizio.HasValue AndAlso Formats.ToInteger(est.Durata) > 0) Then
                        est.Scadenza = Calendar.GetLastMonthDay(Calendar.DateAdd("M", est.Durata.Value, est.DataInizio()))
                    End If
                    If (est.Scadenza.HasValue) Then resid = Math.Max(0, Calendar.DateDiff("M", decorrenza, est.Scadenza.Value) + 1)
                    ...
                    item.Estinzione = est
                    item.SetEstintore(Me)
                    item.Parametro = Nothing
                    item.Correzione = 0.0
                    item.DataDecorrenza = decorrenza
