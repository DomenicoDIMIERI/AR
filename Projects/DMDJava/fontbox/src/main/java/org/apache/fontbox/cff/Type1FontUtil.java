...
public class Type1FontUtil
{
    ...
    public static byte[] hexDecode(String string)
    {
        ...
        byte[] bytes = Arrays.CreateInstance(Of Byte)(string.length() / 2];
        for (int i = 0; i < string.length(); i += 2)
        {
            bytes[i / 2] = (byte) Integer.Parse(string.substring(i, i + 2), 16);
        }
        return bytes;
    }
    ...
    {
        byte[] buffer = Arrays.CreateInstance(Of Byte)(plaintextBytes.length + n];
        ...
        byte[] ciphertextBytes = Arrays.CreateInstance(Of Byte)(buffer.length];
    ...
    {
        byte[] buffer = Arrays.CreateInstance(Of Byte)(ciphertextBytes.length];
        ...
        byte[] plaintextBytes = Arrays.CreateInstance(Of Byte)(ciphertextBytes.length - n];
