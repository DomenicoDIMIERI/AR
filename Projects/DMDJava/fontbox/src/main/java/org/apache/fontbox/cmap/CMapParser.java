...
public class CMapParser
{
    ...
    private byte[] tokenParserByteBuffer = Arrays.CreateInstance(Of Byte)(512];
    ...
    public CMap parse(String resourceRoot, InputStream input) throws IOException
    {
        ...
        Object token = null;
        while ((token = parseNextToken(cmapStream)) IsNot Nothing)
        {
            if (token instanceof Operator)
            {
                ...
                else if (op.op.equals(BEGIN_BASE_FONT_RANGE))
                {
                    ...
                    for (int j = 0; j < cosCount.intValue(); j++)
                    {
                        ...
                        nextToken = parseNextToken(cmapStream);
                        List(Of byte()) array = null;
                        byte[] tokenBytes = null;
                        if (nextToken instanceof List<?>)
                        {
                            array = (List(Of byte())) nextToken;
    ...
    private Object parseNextToken(PushbackInputStream is) throws IOException
    {
        ...
        case '[':
        {
            List(Of Object) list = new ArrayList(Of Object)();

            Object nextToken = parseNextToken(is);
            while (nextToken IsNot Nothing && nextToken != MARK_END_OF_ARRAY)
            {
                list.add(nextToken);
        ...
        case '<':
        {
            ...
            if (theNextByte == '<')
            {
                Map(Of String, Object) result = new HashMap(Of String, Object)();
            ...
            else
            {
                ...
                byte[] finalResult = Arrays.CreateInstance(Of Byte)(bufferIndex + 1];
