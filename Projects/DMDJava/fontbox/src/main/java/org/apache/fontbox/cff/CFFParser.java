...
public class CFFParser
{
     ...
     */
    public List(Of CFFFont) parse(byte[] bytes) throws IOException
    {
        ...
        if (TAG_OTTO.equals(firstTag))
        {
            ...
            for (int q = 0; q < numTables; q++)
            {
                ...
                {
                    cffFound = true;
                    byte[] bytes2 = Arrays.CreateInstance(Of Byte)((int) length];
        ...
        List(Of CFFFont) fonts = new ArrayList(Of CFFFont)();
    ...
    private static Integer readIntegerNumber(CFFDataInput input, int b0) throws IOException
    {
        if (b0 == 28)
        {
            ...
            return NInteger.valueOf((short) (b1 << 8 | b2));
        }
        else if (b0 == 29)
        {
            int b1 = input.readUnsignedByte();
            ...
            return NInteger.valueOf(b1 << 24 | b2 << 16 | b3 << 8 | b4);
        }
    ...
    private CFFFont parseFont(int index) throws IOException
    {
        ...
        if (syntheticBaseEntry IsNot Nothing)
        {
            throw new IOException("Synthetic Fonts are not supported");
        }

        DictData.Entry rosEntry = topDict.getEntry("ROS");
        if (rosEntry IsNot Nothing)
        {
            font = new CFFFontROS();
        ...
        font.addValueToTopDict(
                ...
                getArray(
                        topDict,
                        "FontBBox",
                        Arrays.<Number> asList(NInteger.valueOf(0), NInteger.valueOf(0), NInteger.valueOf(0),
                                NInteger.valueOf(0))));
        ...
        CFFCharset charset;
        int charsetId = charsetEntry IsNot Nothing ? charsetEntry.getNumber(0).intValue() : 0;
        if (charsetId == 0)
        {
        ...
        int[] gids = Arrays.CreateInstance(Of Integer)(charStringsIndex.getCount()];
        List(Of CFFCharset.Entry) glyphEntries = charset.getEntries();
        ...
        int encodingId = encodingEntry IsNot Nothing ? encodingEntry.getNumber(0).intValue() : 0;
        if (encodingId == 0 || rosEntry IsNot Nothing) // --- ROS uses StandardEncoding
        {
        ...
        if (rosEntry IsNot Nothing)
        {
            ...
            List(Of Map(Of String, Object)) privateDictionaries = new LinkedList(Of Map(Of String, Object))();
            List(Of Map(Of String, Object)) fontDictionaries = new LinkedList(Of Map(Of String, Object))();
            ...
            for (int i = 0; i < fdIndex.getCount(); ++i)
            {
                ...
                Map(Of String, Object) fontDictMap = new LinkedHashMap(Of String, Object)();
                ...
                Map(Of String, Object) privDict = new LinkedHashMap(Of String, Object)();
                ...
                privDict.put("BlueShift", getNumber(privateDict, "BlueShift", NInteger.valueOf(7)));
                privDict.put("BlueFuzz", getNumber(privateDict, "BlueFuzz", NInteger.valueOf(1)));
                ...
                privDict.put("LanguageGroup", getNumber(privateDict, "LanguageGroup", NInteger.valueOf(0)));
                privDict.put("ExpansionFactor", getNumber(privateDict, "ExpansionFactor", Double.valueOf(0.06)));
                privDict.put("initialRandomSeed", getNumber(privateDict, "initialRandomSeed", NInteger.valueOf(0)));
                privDict.put("defaultWidthX", getNumber(privateDict, "defaultWidthX", NInteger.valueOf(0)));
                privDict.put("nominalWidthX", getNumber(privateDict, "nominalWidthX", NInteger.valueOf(0)));

                int localSubrOffset = (Integer) getNumber(privateDict, "Subrs", NInteger.valueOf(0));
            ...
            font.addValueToPrivateDict("defaultWidthX", NInteger.valueOf(1000));
            font.addValueToPrivateDict("nominalWidthX", NInteger.valueOf(0));
        ...
        else
        {
            ...
            font.addValueToPrivateDict("BlueShift", getNumber(privateDict, "BlueShift", NInteger.valueOf(7)));
            font.addValueToPrivateDict("BlueFuzz", getNumber(privateDict, "BlueFuzz", NInteger.valueOf(1)));
            ...
            font.addValueToPrivateDict("LanguageGroup", getNumber(privateDict, "LanguageGroup", NInteger.valueOf(0)));
            ...
            font.addValueToPrivateDict("initialRandomSeed",
                    getNumber(privateDict, "initialRandomSeed", NInteger.valueOf(0)));
            font.addValueToPrivateDict("defaultWidthX", getNumber(privateDict, "defaultWidthX", NInteger.valueOf(0)));
            font.addValueToPrivateDict("nominalWidthX", getNumber(privateDict, "nominalWidthX", NInteger.valueOf(0)));
    ...
    private String getString(DictData dict, String name) throws IOException
    {
        ...
        return (entry IsNot Nothing ? readString(entry.getNumber(0).intValue()) : null);
    }

    private Boolean getBoolean(DictData dict, String name, boolean defaultValue) throws IOException
    {
        DictData.Entry entry = dict.getEntry(name);
        return entry IsNot Nothing ? entry.getBoolean(0) : defaultValue;
    }

    private Number getNumber(DictData dict, String name, Number defaultValue) throws IOException
    {
        DictData.Entry entry = dict.getEntry(name);
        return entry IsNot Nothing ? entry.getNumber(0) : defaultValue;
    }
    ...
    {
        DictData.Entry entry = dict.getEntry(name);
        return entry IsNot Nothing ? entry.getArray() : defaultValue;
    }
    ...
    private Format0Encoding readFormat0Encoding(CFFDataInput dataInput, int format, int[] gids) throws IOException
    {
        ...
        encoding.code = Arrays.CreateInstance(Of Integer)(encoding.nCodes];
            ...
            throws IOException
    {
        ...
        fdselect.fds = Arrays.CreateInstance(Of Integer)(nGlyphs];
    ...
    private static class Format0FDSelect extends CIDKeyedFDSelect
    {
        ...
        public int getFd(int glyph)
        {
            Map(Of String, byte()) charString = owner.getCharStringsDict();
            Set(Of String) keys = charString.keySet();
    ...
    private Format0Charset readFormat0Charset(CFFDataInput dataInput, int format, int nGlyphs) throws IOException
    {
        ...
        charset.glyph = Arrays.CreateInstance(Of Integer)(nGlyphs - 1];
    ...
    private Format1Charset readFormat1Charset(CFFDataInput dataInput, int format, int nGlyphs) throws IOException
    {
        ...
        charset.format = format;
        List(Of Format1Charset.Range1) ranges = new ArrayList(Of Format1Charset.Range1)();
    ...
    private static class DictData
    {
        ...
        private List(Of Entry) entries = null;
        ...
        private Entry getEntry(CFFOperator operator)
        {
            for (Entry entry : entries)
            {
                ...
                if (entry IsNot Nothing && entry.operator IsNot Nothing && entry.operator.equals(operator))
                {
                    return entry;
                }
            }
    ...
    abstract static class EmbeddedEncoding extends CFFEncoding
    {
        ...
        List(Of Supplement) getSupplements()
        {
            if (supplement Is Nothing)
            {
                return Collections.(Of Supplement) emptyList();
            }
